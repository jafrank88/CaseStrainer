#!/usr/bin/env python3
"""
Test sync vs async processing with a PDF file
"""
...
def test_file_processing(filename, force_async=False):
    ...
    with open(filename, 'rb') as f:
        ...
        data = {
            ...
            'force_mode': 'async' if force_async else 'sync'
        }
    ...
    if wa_citations:
        print("\nWashington citation details:")
        for i, cit in enumerate(wa_citations[:3]):
            print(f"  {i+1}. {cit.get('citation')}")
            print(f"     Case: {cit.get('extracted_case_name', 'None')}")
            print(f"     Verified: {cit.get('verified', False)}")
...
def compare_results(sync_result, async_result):
    ...
    print(f"\nWashington Citations:")
    print(f"  Sync WA citations: {sync_result['wa_citations']}")
    print(f"  Async WA citations: {async_result['wa_citations']}")
    print(f"  WA count match: {'✓' if sync_result['wa_citations'] == async_result['wa_citations'] else '✗'}")
    ...
    if (len(sync_cits) == len(async_cits) and
        ...
        sync_result['wa_citations'] == async_result['wa_citations'] and
...
if __name__ == "__main__":
    ...
    print("Testing sync vs async processing with PDF file...")
    print(f"URL: {url}")
    ...
    try:
        # Test sync (forcing sync mode)
        print("\n" + "="*60)
        print("Testing with sync mode forced...")
        sync_result = test_file_processing(filename, force_async=False)
        ...
        # Test async
        print("\n" + "="*60)
        print("Testing with async mode...")
        async_result = test_file_processing(filename, force_async=True)
