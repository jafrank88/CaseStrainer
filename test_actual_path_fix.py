...
with open('motion_test_actual_path.json', 'r') as f:
    data = json.load(f)
...
print("=" * 80)
print("TESTING ACTUAL CODE PATH FIX")
print("=" * 80)
...
for i, cit in enumerate(wl_citations, 1):
    ...
    if start is not None and end is not None:
        ...
        if '2024 WL 1232082' in text_at_pos:
            ...
            # Check for semicolon
            if ';' in context:
                semi_pos = context.find(';')
                print(f"  ✅ Semicolon found in context at relative position {semi_pos}")
            ...
            if 'Doe v. Teachers Council' in cit['extracted_name']:
                ...
                # Show the context to debug
                print(f"  Context: ...{context[:80]}[CITATION]{context[-80:]}...")
            else:
                print(f"  ⚠️  Other name: {cit['extracted_name'][:50]}")
        else:
...
if success_count == len(wl_citations):
    ...
    if correct_names == len(wl_citations):
        ...
        print("⚠️  Semicolon boundary detection needs adjustment")
else:
