...
class EnhancedSyncProcessor:
    ...
    def process_any_input_enhanced(self, input_data: Any, input_type: str, options: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[EnhancedSyncProcessor {request_id}] Data separation enforcement failed: {sep_error}")
            ...
            if cluster_case_name and cluster_case_name != 'Unknown Case':
                ...
                for citation in citations:
                    ...
                    if citation_text in cluster_citations:
                        # CRITICAL: Do NOT overwrite extracted_case_name or extracted_date
                        # These fields must ONLY contain data from the user's document
                        # Only set cluster-specific fields
                        if isinstance(citation, dict):
                            citation['cluster_case_name'] = cluster_case_name
                            citation['cluster_year'] = cluster_year
                        elif hasattr(citation, 'cluster_case_name'):
                            setattr(citation, 'cluster_case_name', cluster_case_name)
                            setattr(citation, 'cluster_year', cluster_year)
    ...
    def _wait_and_merge_verification_results(self, result: Dict[str, Any], request_id: str, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            redis_url = os.getenv('REDIS_URL', 'redis://:***REDACTED_REDIS_PASSWORD***@casestrainer-redis-prod:6379/0')
            redis_conn = Redis.from_url(redis_url)
            queue = Queue('casestrainer', connection=redis_conn)
