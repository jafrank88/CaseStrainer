...
def analyze():
    ...
    try:
        ...
        try:
            ...
            task_id = progress_tracker.start_task(len(progress_steps))
            # Map the request_id to our internal task_id for progress tracking
            progress_tracker.active_tasks[request_id] = progress_tracker.active_tasks.get(task_id)
        ...
        if 'file' in request.files and request.files['file'].filename:
            ...
            if data and isinstance(data, dict):
                ...
                if ((data.get('type') == 'url') and data.get('url')) or (data.get('url') and not data.get('text')):
                    input_data = data['url']
                    input_type = 'url'
                    metadata.update({
                        'input_type': 'url',
                    ...
                    logger.warning(f"[Request {request_id}] JSON payload missing 'text' or 'url' fields: {list(data.keys())}")
                    return jsonify({
        ...
        if input_data is not None and input_type is not None:
            ...
            if input_type == 'url':
                try:
                    ...
                    # FORCE SYNC MODE for URLs to prevent async timeout issues
                    force_mode = 'sync'
                    logger.info(f"[Request {request_id}] 🎯 FORCING SYNC MODE for URL input (text size: {len(normalized_text)} chars)")
            ...
            try:
                ...
                # Use unified pipeline for text inputs, UnifiedInputProcessor for other types
                if precomputed_result is not None and input_type != 'text':
                    # Only use precomputed results for non-text inputs (files, URLs)
                    result = precomputed_result
                elif input_type == 'text' and isinstance(input_data, str):
                    logger.info(f"[Request {request_id}] Using UNIFIED PROCESSING PIPELINE for text input")
                    result = _process_text_input(service, request_id, input_data, source_name='api_text')
                else:
...
def _process_text_input(service, request_id, text, source_name="form-input"):
    ...
    try:
        ...
        if len(text) > TEXT_ASYNC_THRESHOLD:
            logger.info(f"[Text Input {request_id}] Text length {len(text)} exceeds async threshold {TEXT_ASYNC_THRESHOLD} - using async processing")
        ...
        try:
            ...
            logger.info(f"[Text Input {request_id}] Found {len(result.get('citations', []))} citations, {len(result.get('clusters', []))} clusters")
            ...
            logger.warning(f"[Text Input {request_id}] Unified pipeline failed, falling back to service: {pipeline_error}")
            ...
            if isinstance(citation, dict):
                if citation.get('verified') is True:
                    ...
                    if extracted and canonical_name and extracted.lower() not in ['n/a', 'none', 'null', '']:
                        ...
                        logger.info(f"[VALIDATION] Skipping similarity check for failed extraction: '{citation.get('citation')}' (extracted='{extracted}', canonical='{canonical_name}')")
