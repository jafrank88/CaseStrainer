"""
Fast verification configuration for Federal citations (F./F.2d/F.3d)
Includes CourtListener lookup, CaseMine, and Law.Resource.org for Federal citations
"""
...
class FastFederalVerificationConfig:
    """
    Fast verification for Federal citations (F./F.2d/F.3d)
    Uses CourtListener lookup, CaseMine, and Law.Resource.org
    """
    
    def __init__(self):
        # Fast sources for Federal citations
    ...
    async def verify_with_law_resource(self, citation: str,
                                      extracted_case_name: Optional[str] = None,
                                      extracted_date: Optional[str] = None) -> VerificationResult:
        """
        ...
        if not self.is_federal_citation(citation):
            return VerificationResult(
                citation=citation,
                verified=False,
                error="Not a Federal Reporter citation"
            )
        ...
        try:
            ...
            if result.verified:
                logger.info(f"[FAST-FEDERAL] Law.Resource.org succeeded for '{citation}'")
                return result
            else:
                logger.info(f"[FAST-FEDERAL] Law.Resource.org failed for '{citation}'")
                
        except Exception as e:
            logger.warning(f"[FAST-FEDERAL] Law.Resource.org error for '{citation}': {e}")
        
        return VerificationResult(
            citation=citation,
            verified=False,
            error="Law.Resource.org verification failed"
        )
    
    async def verify_citation_fast(self, citation: str, 
                                  extracted_case_name: Optional[str] = None,
                                  extracted_date: Optional[str] = None) -> VerificationResult:
        """
        Fast verification for Federal citations
        ...
        Returns:
            VerificationResult with verification status
        """
        logger.info(f"[FAST-FEDERAL] Starting verification for '{citation}'")
        ...
        try:
            ...
            result = await verifier._verify_with_courtlistener_lookup(
                citation, extracted_case_name, extracted_date
            )
            if result.verified:
                logger.info(f"[FAST-FEDERAL] CourtListener lookup succeeded for '{citation}'")
                return result
            else:
                logger.info(f"[FAST-FEDERAL] CourtListener lookup failed for '{citation}'")
        except Exception as e:
            logger.warning(f"[FAST-FEDERAL] CourtListener lookup error for '{citation}': {e}")
        ...
        try:
            ...
            result = await verifier._verify_with_casemine(
                citation, extracted_case_name, extracted_date, 
                min(self.timeout_per_source, 12.0)
            )
            if result.verified or getattr(result, 'possible_match', False):
                logger.info(f"[FAST-FEDERAL] CaseMine succeeded for '{citation}'")
                return result
            else:
                logger.info(f"[FAST-FEDERAL] CaseMine failed for '{citation}'")
        except Exception as e:
            logger.warning(f"[FAST-FEDERAL] CaseMine error for '{citation}': {e}")
        ...
        # All sources failed
        logger.warning(f"[FAST-FEDERAL] All fast sources failed for '{citation}'")
        return VerificationResult(
            citation=citation,
            verified=False,
            error="Fast verification failed - all sources failed"
        )
    
    async def verify_citations_batch_fast(self, citations: List[str],
                                         extracted_case_names: Optional[List[str]] = None,
                                         extracted_dates: Optional[List[str]] = None) -> List[VerificationResult]:
        """
        Batch verification for Federal citations
        ...
        """
        logger.info(f"[FAST-FEDERAL] Starting batch verification for {len(citations)} citations")
        
        # Prepare data
        case_names = extracted_case_names or [None] * len(citations)
        dates = extracted_dates or [None] * len(citations)
        
        # First try CourtListener batch lookup
        try:
            from src.unified_verification_master import UnifiedVerificationMaster
            verifier = UnifiedVerificationMaster()
            ...
            # Check which citations need additional sources
            results = []
            additional_needed = []
            ...
            for i, citation in enumerate(citations):
                ...
                if result and result.verified:
                    ...
                    results.append(VerificationResult(
                        citation=citation,
                        verified=False,
                        error="CourtListener lookup failed"
                    ))
            ...
            if additional_needed:
                logger.info(f"[FAST-FEDERAL] Trying additional sources for {len(additional_needed)} failed citations")
                ...
                for idx, citation, case_name, date in additional_needed:
                    try:
                        ...
                        logger.warning(f"[FAST-FEDERAL] Additional sources failed for '{citation}': {e}")
            
            return results
            
        except Exception as e:
            logger.error(f"[FAST-FEDERAL] Batch verification failed: {e}")
...
# Singleton instance
fast_federal_verification = FastFederalVerificationConfig()
