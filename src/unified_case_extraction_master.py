...
class UnifiedCaseExtractionMaster:
    ...
    def _extract_case_name_backward_from_v(self, text: str, v_position: int, debug: bool = False) -> Optional[str]:
        ...
        legal_stopwords = {
            'of', 'the', 'in', 're', 'ex', 'and', 'for', 'by', 'at', 'on', 'to', 
            'de', 'la', 'du', 'von', 'van', 'der', 'del', 'al', 'parte', 'matter',
            'an', 'a', 'as', 'or', 'nor', 'but', 'yet', 'so', 'with', 'from'
        }
        
        # Legal abbreviations (case-insensitive matching)
        # These are words that look lowercase but are actually abbreviations
        legal_abbreviations = {
            'inc', 'llc', 'llp', 'corp', 'co', 'ltd', 'pc', 'lp', 'na', 'plc',
            "dep't", "dept", "cmty", "comm'n", "commn", "ass'n", "assn", 
            "gov't", "govt", "cty", "ct", "app", "assocs", "bros", "ins",
            "mfg", "mkt", "prod", "servs", "sys", "int'l", "intl", "nat'l", "natl",
            "elec", "eng'g", "engg", "indus", "cnty", "twp", "univ", "hosp",
            "med", "ctr", "grp", "mgmt", "dev", "prop", "props", "hum", "res",
            "tech", "fin", "svcs", "admin", "ops", "commr", "comr", "supt",
            "u.s", "n.a", "p.c", "l.p", "l.l.c", "d.b.a", "dba"
        }
        ...
        if current_word and not boundary_hit:
            ...
            should_include = (
                (word_clean and word_clean[0].isupper()) or
                word_lower in legal_stopwords or
                word_lower.rstrip('.') in legal_abbreviations
            )
            if should_include:
                words.insert(0, word)
    ...
    def extract_case_name_and_date(
        ...
        if debug or force_debug:
            logger.warning(f"[DEBUG] MASTER_EXTRACT: Starting unified extraction for '{citation}' at {start_index}-{end_index}")
            if force_debug:
        ...
        if citation:
            ...
            if match:
                ...
                logger.warning(f"[SUCCESS] [SIMPLE-FORMAT] Extracted from standalone citation: '{extracted_name}' ({extracted_year})")
        ...
        if citation:
            ...
            if start_index is not None:
                ...
                try:
                    result = self._extract_special_citation_formats(text, citation, start_index, bool(debug or force_debug))
                    logger.error(f"[SPECIAL-FORMATS] 🔙 Strategy -0.5 RETURNED for '{citation}' - result type: {type(result)}")
                    if result and result.case_name and result.case_name != 'N/A':
                        logger.error(f"[SUCCESS] ✅ Strategy -0.5 extracted: '{result.case_name}'")
                        return result
                    else:
                        logger.error(f"[SPECIAL-FORMATS] ❌ Strategy -0.5 returned nothing for '{citation}' - result={result}")
        ...
        if citation:
            ...
            if canonical_metadata and canonical_metadata.get('canonical_name'):
                logger.warning(f"[INFO] [CANONICAL-FALLBACK] Using canonical name for failed extraction: {canonical_metadata['canonical_name']}")
                return MasterExtractionResult(
            ...
            for signal in signal_words:
                ...
                if matches:
                    ...
                    if match_end_distance > 100:
                        logger.error(f"[SPECIAL-FORMATS] Pattern 4 SKIP: signal '{signal}' match too far ({match_end_distance} chars from citation)")
                        continue
                    case_name = match.group(1).strip()
                    logger.error(f"[SPECIAL-FORMATS] Pattern 4 (signal '{signal}') raw match (last of {len(matches)}): '{case_name}'")
    ...
    def _extract_with_comma_anchor(self, text: str, citation: str, start_index: int, debug: bool) -> Optional[MasterExtractionResult]:
        ...
        if prev_citations:
            ...
            print(f"[PHASE6-PREV-CIT] Found previous citation, truncated pre_citation_text to: '{pre_citation_text}'", flush=True)
        ...
        for pattern, priority, pattern_type in patterns:
            for match in re.finditer(pattern, context_cleaned, re.IGNORECASE):
                ...
                if has_intervening_citation:
                    logger.error(f"[FIX DEC 2025] INTERVENING CITATION detected after '{case_name[:30]}' - applying boundary penalty")
    ...
    def _extract_with_position(self, text: str, citation: str, start_index: int, end_index: int, debug: bool) -> Optional[MasterExtractionResult]:
        ...
        if prev_citations:
            ...
            if debug:
                logger.warning(f"[PREV-CIT-BOUNDARY] Found prev citation at {last_prev_cit.start()}-{last_prev_cit.end()}")
        ...
        for i, pattern in enumerate(self.case_name_patterns):
            ...
            if match:
                ...
                if start_index is not None and end_index is not None:
                    ...
                    if min_distance > 100:
                        if debug:
                            logger.warning(f"🚫 Position validation failed: match is {min_distance} chars from citation")
                ...
                if case_name and len(case_name.strip()) > 3:
                    ...
                    if distance_from_citation > 150:  # Increased from 100 to 150 chars
                        print(f"[PHASE6-REJECT-PROXIMITY] Rejected: distance {distance_from_citation} > 150", flush=True)
                        if debug:
    ...
    def _clean_case_name(self, case_name: str, context: Optional[str] = None) -> str:
            ...
            if context:
                ...
                if defendant_match:
                    ...
                    full_case_match = re.search(
                        r'([A-Z][A-Za-z\s&\'\.\-,]+)\s+v\.\s+' + re.escape(defendant[:20]),
                    ...
                    if full_case_match:
                        ...
                        if plaintiff and len(plaintiff) > 2:
                            cleaned = f"{plaintiff} v. {defendant}"
                            logger.error(f"[TRUNCATION-REPAIR] Repaired missing plaintiff: 'v. {defendant}' -> '{cleaned}'")
        ...
        partial_truncation_match = re.match(r'^([A-Z][a-z]+)\s+(Co\.?|Corp\.?|Inc\.?)\s+v\.\s+(.+)$', cleaned, re.IGNORECASE)
        if partial_truncation_match and context:
            ...
            full_company_pattern = rf'([A-Z][A-Za-z\s&\'\.\-,]+\s+{re.escape(short_name)}\s+{re.escape(suffix)}\.?)\s+v\.'
            ...
            if full_company_match:
                ...
                logger.error(f"[TRUNCATION-REPAIR] Repaired partial truncation: '{short_name} {suffix}' -> '{full_plaintiff}'")
        ...
        if context and ' v. ' in cleaned:
            ...
            if v_pos > 0:
                ...
                if len(first_party) < 20 and not any(word in first_party.lower() for word in ['state', 'united states', 'city of', 'county of']):
                    ...
                    if full_name_match:
                        ...
                        full_first_party = re.sub(r'^(?:See|Citing|In|The)\s+', '', full_first_party, flags=re.IGNORECASE).strip()
