    ...
    try:
        ...
        if input_type in ['text', 'url']:
            ...
            if input_type == 'text':
                ...
                try:
                    vm.update_progress(task_id, processed=0, total=0, message='Downloading and extracting text from URL...')
                except Exception:
                    pass
                ...
                try:
                    ...
                    try:
                        vm.update_progress(task_id, processed=0, total=0, message='Downloaded document, extracting text...')
                    except Exception:
                        pass
            ...
            if not locals().get('skip_full_processing', False):
                ...
                try:
                    ...
                    try:
                        vm.update_progress(task_id, processed=0, total=0, message='Extracting citations from document...')
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        def update_verification_progress(processed_count, status, message):
                            ...
                            try:
                                ...
                                if current_total == 0 or current_total == 100:
                                    ...
                                    if msg_match:
                                        ...
                                        if extracted_total > 0:
                                            ...
                                            logger.info(f"[TASK:{task_id}] Extracted total from message: {current_total}")
                                ...
                                vm.update_progress(task_id, processed=processed_count, total=current_total, message=message)
                                logger.info(f"[TASK:{task_id}] Progress callback: {processed_count}/{current_total} citations - {message}")
                            except Exception as e:
                        ...
                        if clusters_list and citations_list:
                            logger.info(f"[TASK:{task_id}] Post-processing clusters to populate canonical data from citations")
                            ...
                            for cluster in clusters_list:
                                ...
                                if best_extracted_name and best_canonical_name and best_extracted_name != 'N/A' and best_canonical_name != 'N/A':
                                    import re
                                    ...
                                    if ext_second and can_second and ext_second != can_second:
                                        ...
                                        if overlap < 2:
                                            logger.info(f"[TASK:{task_id}] Cascading fix: '{best_extracted_name}' -> '{best_canonical_name}' (second party: '{ext_second}' != '{can_second}')")
                                            ...
                                            for cit in cluster_cits:
                                                if isinstance(cit, dict):
                                                    ...
                                                    if cit_ext_name and (cit_ext_name == old_name or 
                                                        get_second_party(cit_ext_name) == ext_second):
                                                        cit['extracted_case_name'] = best_canonical_name
                                                        # Recalculate name_mismatch
                                                        cit['name_mismatch'] = False  # Names now match
                                                        logger.debug(f"[TASK:{task_id}] Fixed citation extracted_case_name: '{cit_ext_name}' -> '{best_canonical_name}'")
                                ...
                                has_name_mismatch = any(
                                    ...
                                    for c in cluster_cits if isinstance(c, dict)
                                )
                                cluster['has_name_mismatch'] = has_name_mismatch
                                cluster['verified'] = any_verified
                            
                            logger.info(f"[TASK:{task_id}] Post-processing complete: updated {len(clusters_list)} clusters")
                        ...
                        if clusters_list:
                                        ...
                                        if old_date != cluster_date:
                                            ...
                                            if canonical_date:
                                                ...
                                                if ext_year_match and can_year_match:
                                                    ...
                                                    year_diff = abs(int(ext_year_match.group(0)) - int(can_year_match.group(0)))
                            ...
                            if date_sync_count > 0:
                                logger.info(f"[TASK:{task_id}] Synchronized {date_sync_count} citation extracted_dates with cluster dates")
                        ...
                        if clusters_list and text:
                            ...
                            for cluster in clusters_list:
                                ...
                                first_party_key = re.sub(r'\b(the|of|in|and|inc|corp|llc|ltd|co)\b', '', first_party.lower(), flags=re.IGNORECASE).strip()
                                ...
                                context_normalized = context.replace('–', '-').replace('—', '-').replace('\u2013', '-').replace('\u2014', '-')
                                first_party_normalized = first_party_key.replace('–', '-').replace('—', '-').replace('\u2013', '-').replace('\u2014', '-')
                                if first_party_normalized not in context_normalized:
                                    # Extracted name doesn't appear near citation - likely wrong
                                    logger.warning(f"[TASK:{task_id}] Detected phantom extraction: '{ext_name}' not found near citation at {cit_pos}")
                                    # Try to find correct name from context
                                    v_match = re.search(r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+v\.\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)', text[context_start:context_end])
                                    if v_match:
                        ...
                        if clusters_list:
                            import re
                            fragment_pattern = re.compile(r'^(Inc\.?|Corp\.?|LLC|L\.L\.C\.|Ltd\.?|Co\.?|Ass\'?n|Assoc\.?|Org\.?)\s+v\.?\s+', re.IGNORECASE)
                            for cluster in clusters_list:
                                if not isinstance(cluster, dict):
                                    continue
                                ext_name = cluster.get('extracted_case_name', '')
                                if ext_name and fragment_pattern.match(str(ext_name)):
                                    ...
                                    if canonical and canonical != 'N/A':
                                        ...
                                        logger.info(f"[TASK:{task_id}] Fragment '{ext_name}' - using canonical for display only: '{canonical}'")
                                    else:
                        ...
                        if clusters_list and len(clusters_list) > 1:
                            ...
                            def get_key_words(name):
                                ...
                                common = {'v', 'the', 'of', 'and', 'in', 'inc', 'corp', 'co', 'llc', 'ltd', 'vs', 'city', 'state', 'county', 'dept', 'department'}
                                    ...
                                    for i in range(len(indices)):
                                        ...
                                        for j in range(i + 1, len(indices)):
                                            ...
                                            if words1 and words2:
                                                ...
                                                if len(overlap) >= 2:
                                                    logger.info(f"[TASK:{task_id}] Merging clusters by name similarity: '{name1}' + '{name2}' (shared: {overlap})")
                                                    # Merge j into i
                                                    leader = c1
                                                    other = c2
                            ...
                            if to_remove:
                                ...
                                logger.info(f"[TASK:{task_id}] Removed {len(to_remove)} duplicate clusters, now {len(clusters_list)}")
                        ...
                        if clusters_list and citations_list and len(clusters_list) > 1:
                            ...
                            if len(na_singleton_indices) >= 2:
                                logger.info(f"[TASK:{task_id}] Found {len(na_singleton_indices)} N/A singletons to check for parallel merging")
                                        ...
                                        if distance < 100:
                                            if _are_parallel_reporter_types(cit1_text, cit2_text):
                                                if _citations_compatible_for_parallel(cit1_text, cit2_text):
                                                    logger.info(f"[TASK:{task_id}] N/A parallel match: '{cit1_text}' + '{cit2_text}' (distance={distance})")
                                ...
                                for from_idx, into_idx in na_to_merge:
                                    ...
                                    for fc in from_citations:
                                        ...
                                        existing = [c.get('citation', '') if isinstance(c, dict) else str(c) for c in into_citations]
                                ...
                                if na_merged_indices:
                                    clusters_list = [c for i, c in enumerate(clusters_list) if i not in na_merged_indices]
                                    logger.info(f"[TASK:{task_id}] Merged {len(na_merged_indices)} N/A singletons, now {len(clusters_list)} clusters")
                                        ...
                                        for om in other_members:
                                            ...
                                            if distance < 100:
                                                ...
                                                if not _citations_compatible_for_parallel(member_cit, other_cit):
                                                    logger.warning(f"[TASK:{task_id}] REJECTED parallel: '{member_cit}' + '{other_cit}' - different jurisdictions")
                                                    continue
                                                ...
                                                if text:
                                                    ...
                                                    end_pos = max(member_pos, other_pos) + 20  # Add 20 to include citation text
                                                    between_text = text[start_pos:end_pos] if len(text) > end_pos else text[start_pos:]
                                                    logger.info(f"[TASK:{task_id}] Checking for case history between '{member_cit}' and '{other_cit}': '{between_text[:100]}...'")
                                                    if _has_case_history_signal_between(text, member_pos, other_pos + 20):
                                                        logger.warning(f"[TASK:{task_id}] REJECTED parallel: '{member_cit}' + '{other_cit}' - case history signal between")
                                                        continue
                                                else:
                                                    logger.warning(f"[TASK:{task_id}] No text available for case history check")
                                                logger.info(f"[TASK:{task_id}] Found parallel: '{member_cit}' near '{other_cit}' (distance={distance})")
                                                to_merge.append((i, j))
                                                break
                            ...
                            for singleton_idx, target_idx in to_merge:
                                ...
                                for cit in singleton_citations:
                                    ...
                                    existing_cits = [c.get('citation', '') if isinstance(c, dict) else str(c) for c in target_citations]
                                ...
                                logger.info(f"[TASK:{task_id}] Merged singleton cluster {singleton_idx} into {target_idx} (now {len(target_citations)} citations)")
                            
                            if merged_indices:
                                clusters_list = [c for i, c in enumerate(clusters_list) if i not in merged_indices]
                                logger.info(f"[TASK:{task_id}] Merged {len(merged_indices)} parallel citations, now {len(clusters_list)} clusters")
                        ...
                        if clusters_list and len(clusters_list) > 1:
                            ...
                            for name, indices in name_groups.items():
                                if len(indices) > 1:
                                    logger.info(f"[TASK:{task_id}] Merging {len(indices)} unverified clusters for '{name}'")
                                    ...
                                    for idx in indices[1:]:
                                        ...
                                        for cit in other_citations:
                                            cit_str = cit.get('citation', '') if isinstance(cit, dict) else str(cit)
                                            existing_cits = [c.get('citation', '') if isinstance(c, dict) else str(c) for c in leader_citations]
                                            if cit_str not in existing_cits:
                            ...
                            if to_remove:
                                clusters_list = [c for i, c in enumerate(clusters_list) if i not in to_remove]
                                logger.info(f"[TASK:{task_id}] Merged unverified same-name clusters, now {len(clusters_list)} clusters")
                            ...
                            for cluster in clusters_list:
                                    ...
                                    if isinstance(cit, dict):
                                        ...
                                        if cit_text:
                                            ...
                                            if pos >= 0:
                                                ...
                                                logger.debug(f"[TASK:{task_id}] Found '{cit_text}' at actual position {pos}-{end}")
                                ...
                                for i in range(len(cits_with_pos) - 1):
                                    ...
                                    if curr_end > 0 and next_start > curr_end:
                                        ...
                                        curr_cit = cits_with_pos[i][2].get('citation', '') if isinstance(cits_with_pos[i][2], dict) else ''
                                        next_cit = cits_with_pos[i + 1][2].get('citation', '') if isinstance(cits_with_pos[i + 1][2], dict) else ''
                                        logger.info(f"[TASK:{task_id}] Checking text between '{curr_cit}' and '{next_cit}': '{text_between[:80]}...'")
                                        
                                        # Check for case history signals
                                        if _has_case_history_signal_between(text, curr_end, next_start):
                                            logger.info(f"[TASK:{task_id}] Found case history signal between '{curr_cit}' and '{next_cit}': splitting cluster")
                                ...
                                if not split_points:
                                    ...
                                    logger.info(f"[TASK:{task_id}] Split cluster into {len(split_points) - 1} parts due to case history signals")
                            
                            if len(new_clusters) != len(clusters_list):
                                logger.info(f"[TASK:{task_id}] Cluster splitting: {len(clusters_list)} -> {len(new_clusters)} clusters")
                        ...
                        if citations_list and not clusters_list:
                            try:
                                ...
                                if recomputed:
                                    ...
                                    logger.warning(f"[TASK:{task_id}] Fallback recompute produced {len(clusters_list)} clusters")
                                else:
                        ...
                        logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Starting consistency pass for {len(citations_list)} citations")
                        if citations_list:
                            ...
                            logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Built lookup with {len(citation_lookup)} citations")
                            consistency_fixed = 0
                            groups_found = 0
                            ...
                            logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Found {len(proximity_groups)} proximity-based groups")
                            ...
                            for group_cits in proximity_groups:
                                ...
                                has_unverified = any(gc.get('verified') != True and not gc.get('true_by_parallel', False) for gc in group_cits)
                                        ...
                                        if gc.get('verified') != True and not gc.get('true_by_parallel', False):
                                            ...
                                            if source_citation:
                                                if source_citation.get('canonical_name') and not gc.get('canonical_name'):
                                                    ...
                                                    if '_original_obj' in gc:
                                                        gc['_original_obj'].canonical_name = source_citation.get('canonical_name')
                                                if source_citation.get('canonical_date') and not gc.get('canonical_date'):
                                                    gc['canonical_date'] = source_citation.get('canonical_date')
                                                    if '_original_obj' in gc:
                                                        gc['_original_obj'].canonical_date = source_citation.get('canonical_date')
                                                if source_citation.get('canonical_url') and not gc.get('canonical_url'):
                                                    gc['canonical_url'] = source_citation.get('canonical_url')
                                                    if '_original_obj' in gc:
                                                        gc['_original_obj'].canonical_url = source_citation.get('canonical_url')
                                            consistency_fixed += 1
                                            logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Fixed (proximity) {gc.get('citation')} now true_by_parallel")
                                    ...
                                    for gc in group_citations:
                                        if gc.get('verified') != True and not gc.get('true_by_parallel', False):
                                            ...
                                            if source_citation.get('canonical_url') and not gc.get('canonical_url'):
                                                gc['canonical_url'] = source_citation.get('canonical_url')
                                            consistency_fixed += 1
                                            logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Fixed {gc.get('citation')} now true_by_parallel")
                                
                                visited_cites.update(group)
                            
                            logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Found {groups_found} parallel groups, fixed {consistency_fixed} citations")
                                ...
                                if not has_verified:
                                    ...
                                    for gc in group_cits:
                                        if gc.get('true_by_parallel', False):
                                            ...
                                            orphan_cleared += 1
                                            logger.info(f"[TASK:{task_id}] PARALLEL-ORPHAN: Cleared true_by_parallel from {gc.get('citation')} (no verified source)")
                            
                            if orphan_cleared > 0:
                                logger.info(f"[TASK:{task_id}] PARALLEL-ORPHAN: Cleared {orphan_cleared} orphaned true_by_parallel citations")
                                        ...
                                        for i, cit in enumerate(cluster_citations):
                                            if isinstance(cit, dict):
                                                ...
                                                if cite_text in citation_lookup:
                                                    ...
                                                    if updated.get('true_by_parallel') and not cit.get('true_by_parallel'):
                                                        cit['true_by_parallel'] = True
                                                        if updated.get('canonical_name') and not cit.get('canonical_name'):
                                                            cit['canonical_name'] = updated.get('canonical_name')
                                                        if updated.get('canonical_date') and not cit.get('canonical_date'):
                                                            cit['canonical_date'] = updated.get('canonical_date')
                                                        if updated.get('canonical_url') and not cit.get('canonical_url'):
                                                            cit['canonical_url'] = updated.get('canonical_url')
                                                        cluster_updates += 1
                                                    elif not updated.get('true_by_parallel') and cit.get('true_by_parallel'):
                                                        ...
                                                        cluster_updates += 1
                                logger.info(f"[TASK:{task_id}] PARALLEL-CONSISTENCY: Updated {cluster_updates} citations in clusters")
                                ...
                                if not has_verified_in_cluster:
                                    ...
                                    for cit in cluster_cits:
                                        if isinstance(cit, dict) and cit.get('true_by_parallel', False):
                                            ...
                                            logger.info(f"[TASK:{task_id}] CLUSTER-ORPHAN: Cleared true_by_parallel from {cit.get('citation')} (no verified in cluster)")
                            ...
                            if orphaned_citations:
                                ...
                                if top_level_cleared > 0:
                                    logger.info(f"[TASK:{task_id}] CLUSTER-ORPHAN: Also cleared {top_level_cleared} from top-level citations")
                            
                            if final_orphan_cleared > 0:
                                logger.info(f"[TASK:{task_id}] CLUSTER-ORPHAN: Cleared {final_orphan_cleared} orphaned true_by_parallel flags at cluster level")
                        
                        if len(citations_list) > 0:
                            logger.info(f"[TASK:{task_id}] First citation sample: {citations_list[0] if citations_list else 'None'}")
                        else:
                            logger.warning(f"[TASK:{task_id}] WARNING: No citations found in pipeline result!")
                            logger.warning(f"[TASK:{task_id}] Pipeline result keys: {list(pipeline_result.keys()) if isinstance(pipeline_result, dict) else 'Not a dict'}")
                            logger.warning(f"[TASK:{task_id}] Raw citations type: {type(citations_raw)}, length: {len(citations_raw) if citations_raw else 0}")
                        ...
                        try:
                            if len(citations_list) > 0:
                                ...
                                if enable_verification:
                                    ...
                                    vm.update_progress(task_id, processed=0, total=total_cites, message=f'Extracted {total_cites} citations, {len(clusters_list)} clusters - Starting verification...')
                                    logger.info(f"[TASK:{task_id}] Updated verification status: {total_cites} citations found, verification starting (processed=0)")
                                else:
                                    # No verification, so all citations are "processed" (just extracted)
                                    vm.update_progress(task_id, processed=total_cites, total=total_cites, message=f'Extracted {total_cites} citations, {len(clusters_list)} clusters - Processing complete!')
                                    logger.info(f"[TASK:{task_id}] Updated verification status: {total_cites} citations found (no verification)")
                            else:
                                vm.update_progress(task_id, processed=0, total=0, message='Processing complete - no citations found')
                        ...
                        try:
                            ...
                            if elapsed > 90:
                                ...
                                if targets:
                                    ...
                                    enriched = _verify_enhanced(targets, text, task_id, 'url', {'source': 'worker_fallback'})
                                    if isinstance(enriched, dict) and enriched.get('success') and enriched.get('citations'):
                                        ...
                                        new_verified = sum(1 for c in citations_list if isinstance(c, dict) and c.get('verified'))
                                        logger.info(f"[TASK:{task_id}] Fallback verification complete: {new_verified}/{len(citations_list)} now verified")
                                    else:
                    ...
                    if len(result.get('citations', [])) > 0:
                        logger.info(f"[TASK:{task_id}]   - First citation keys: {list(result['citations'][0].keys()) if isinstance(result['citations'][0], dict) else 'Not a dict'}")
                    ...
                    try:
                        ...
                        logger.error(f"[TASK:{task_id}] Problematic citations: {[type(c).__name__ for c in citations_list[:5]]}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f"[TASK:{task_id}] Failed to save complete result (non-critical): {complete_err}")
...
def main():
    ...
    logger.info(f"Redis URL: {os.environ.get('REDIS_URL', 'redis://:***REDACTED_REDIS_PASSWORD***@casestrainer-redis-prod:6379/0')}")
    logger.info("=" * 80)
