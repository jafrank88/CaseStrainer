...
class EnhancedAsyncManager:
    ...
    async def submit_job(
        self, input_data: Dict[str, Any], processor_func: Callable, job_id: str = None, priority: int = 0
    ) -> str:
        ...
        # Create job info
        job_info = JobInfo(job_id=job_id, status=JobStatus.PENDING, created_at=datetime.now(), input_data=input_data)
    ...
    async def _process_job(self, job_id: str, input_data: Dict[str, Any], processor_func: Callable):
        ...
        async with self.job_semaphore:
            try:
                ...
                result = await asyncio.wait_for(
                    self._run_processor_with_progress(job_id, input_data, processor_func), timeout=self.job_timeout
                )
                ...
                if job_info:
                    ...
                    job_info.processing_time = (
                        (job_info.completed_at - job_info.started_at).total_seconds() if job_info.started_at else None
                    )
                    await self._store_job_info(job_info)
    ...
    async def _run_processor_with_progress(
        self, job_id: str, input_data: Dict[str, Any], processor_func: Callable
    ) -> Dict[str, Any]:
        ...
        if asyncio.iscoroutinefunction(processor_func):
            ...
            if "progress_callback" in processor_func.__code__.co_varnames:
                ...
                result = await loop.run_in_executor(self.thread_executor, processor_func, input_data, progress_callback)
            else:
                loop = asyncio.get_event_loop()
                result = await loop.run_in_executor(self.thread_executor, processor_func, input_data)

        return result

    async def update_job_progress(self, job_id: str, progress: float, step: str = "", message: str = ""):
            ...
            try:
                ...
                if self.redis_client:
                    ...
                    for job_key in job_keys:
                        ...
                        if job_data:
                            job_info = pickle.loads(job_data)
                            if job_info.completed_at and job_info.completed_at < cutoff_time:
...
async def get_async_manager(
    redis_url: str = "redis://localhost:6379/0", max_concurrent_jobs: int = 10, job_timeout: int = 300
    ...
    if _global_manager is None:
        _global_manager = EnhancedAsyncManager(
            redis_url=redis_url, max_concurrent_jobs=max_concurrent_jobs, job_timeout=job_timeout
        )
...
async def submit_processing_job(input_data: Dict[str, Any], processor_func: Callable, job_id: str = None) -> str:
