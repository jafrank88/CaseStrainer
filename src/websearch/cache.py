...
class CacheManager:
    ...
    def get(self, *args) -> Optional[Any]:
        ...
        cursor.execute("SELECT value, timestamp, ttl_hours FROM search_cache WHERE key = ?", (key,))
    ...
    def get_url_status(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        cursor.execute("SELECT status, status_code, last_checked, content_hash FROM url_status WHERE url = ?", (url,))
        result = cursor.fetchone()
        conn.close()
    ...
    def set_url_status(
        self, url: str, status: str, status_code: Optional[int] = None, content_hash: Optional[str] = None
    ):
