...
from .cache import cache_manager, MemoryCache
...
class OptimizedCitationExtractor(ICitationExtractor):
    ...
    def _compile_patterns(self):
        ...
        self._compiled_patterns = {
            "us_supreme_court": re.compile(r"\b(\d+)\s+U\.S\.\s+(\d+)\s*\((\d{4})\)", re.IGNORECASE),
            "federal_reporter": re.compile(
                r"\b(\d+)\s+F\.\s*(?:2d|3d)?\s+(\d+)\s*\([^)]*(\d{4})[^)]*\)", re.IGNORECASE
            ),
            "supreme_court_reporter": re.compile(r"\b(\d+)\s+S\.\s*Ct\.\s+(\d+)\s*\((\d{4})\)", re.IGNORECASE),
            "lawyers_edition": re.compile(r"\b(\d+)\s+L\.\s*Ed\.\s*(?:2d)?\s+(\d+)\s*\((\d{4})\)", re.IGNORECASE),
            "case_name": re.compile(
                r"\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+v\.\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)", re.IGNORECASE
            ),
        }
...
class OptimizedCitationClusterer(ICitationClusterer):
    ...
    def _detect_parallels_in_group(self, group: List[CitationResult], text: str) -> None:
        ...
        has_parallel_indicators = any(re.search(pattern, group_text, re.IGNORECASE) for pattern in parallel_indicators)
    ...
    def _are_likely_parallel(self, citation1: CitationResult, citation2: CitationResult) -> bool:
        ...
        if (
            ...
            and citation1.extracted_case_name.lower() == citation2.extracted_case_name.lower()
        ):
            return True

        if (
            citation1.extracted_date
