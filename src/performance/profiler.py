...
class PerformanceBenchmark:
    ...
    def add_test_case(self, name: str, text: str, expected_citations: Optional[int] = None):
        ...
        self.test_cases.append(
            {"name": name, "text": text, "expected_citations": expected_citations, "text_length": len(text)}
        )
    ...
    def print_benchmark_results(self):
        ...
        print(
            f"{'Test Case':<25} {'Text Len':<10} {'Avg Time':<10} {'Min Time':<10} {'Max Time':<10} {'Chars/sec':<12}"
        )
        print("-" * 80)

        for case_name, metrics in self.results.items():
            print(
                f"{case_name:<25} {metrics['text_length']:<10} "
...
def create_standard_benchmark() -> PerformanceBenchmark:
    ...
    benchmark.add_test_case(
        "small_doc", "Brown v. Board of Education, 347 U.S. 483 (1954) was a landmark case.", expected_citations=1
    )
