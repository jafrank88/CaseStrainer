...
def compare_citations(toa_citations: List[Dict], body_citations: List[Dict]) -> Dict[str, Any]:
    ...
    for norm_toa, toa_cite in normalized_toa.items():
        if norm_toa in normalized_body:
            ...
            matches.append({"toa_citation": toa_cite, "body_citation": body_cite, "normalized_text": norm_toa})
        ...
        for i, line in enumerate(toa_lines, 1):
            ...
            if results and isinstance(results, dict) and "citations" in results:
                for citation in results["citations"]:
                    ...
                    if normalize(toa_name or "") != normalize(proc_name or "") or str(toa_year or "") != str(
                        proc_date or ""
                    ):
...
def validate_against_toa(
    body_extractions: List[Dict[str, Any]], toa_extractions: List[Dict[str, Any]]
    ...
    return {
        ...
        "toa_only": [
            ext
            for ext in toa_extractions
            if ext.get("case_name", "").lower() not in {ext.get("case_name", "").lower() for ext in body_extractions}
        ],
    }
...
def get_cases_by_year_range(extractions: List[Dict[str, Any]], start_year: int, end_year: int) -> List[Dict[str, Any]]:
    """
...
def get_cases_by_court(extractions: List[Dict[str, Any]], court_pattern: str) -> List[Dict[str, Any]]:
    """
...
def export_to_csv_format(extractions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for extraction in extractions:
        row = {
            ...
            "court": extraction.get("court"),
            "court_normalized": (
                _normalize_court_name(extraction.get("court") or "") if extraction.get("court") else None
            ),
