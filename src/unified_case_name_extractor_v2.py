...
warnings.warn(
    "WARNING: unified_case_name_extractor_v2.py is DEPRECATED. " "Active code is in src.unified_case_extraction_master",
    DeprecationWarning,
    stacklevel=2,
)
...
class UnifiedCaseNameExtractorV2:
    ...
    def _setup_patterns(self):
        ...
        legal_abbrev = (
            r"(?:U\.S\.|Sec\'y|Gov\'t|Dep\'t|Att\'y|Dist\.|Comm\'r|Ass\'n|Bd\.|Comm\'n|Div\.|Sch\.|Univ\.|Coll\.)"
        )
        ...
        try:
            ...
            for strategy in strategies:
                ...
                if strategy == ExtractionStrategy.VOLUME_BASED:
                    extraction_result = self._extract_volume_based(
                        text, citation or "", citation_start, citation_end, debug
                    )
                elif strategy == ExtractionStrategy.CONTEXT_BASED:
                    extraction_result = self._extract_context_based(
                        text, citation or "", citation_start, citation_end, debug
                    )
                elif strategy == ExtractionStrategy.PATTERN_BASED:
                    extraction_result = self._extract_pattern_based(
                        text, citation or "", citation_start, citation_end, debug
                    )
                ...
                if extraction_result and extraction_result.case_name:
                    ...
                    if debug:
                        logger.info(
                            f" [UNIFIED_EXTRACTOR] Strategy {strategy.value} succeeded: '{result.case_name}' (confidence: {result.confidence})"
                        )
    ...
    def _extract_context_based(
        ...
        try:
            ...
            window_start = (
                paren_start + 1
                if (paren_start != -1 and (paren_end == -1 or paren_start > paren_end))
                else max(0, citation_pos - 200)
            )
            ...
            for pattern_info in sorted(self.patterns, key=lambda x: -x["confidence"]):
                ...
                for match in matches:
                    ...
                    # Combined score
                    score = pattern_info["confidence"] * 0.5 + position_score * 0.3 + length_score * 0.2
    ...
    def _validate_case_name(self, case_name: str, text: str, citation: str) -> Dict[str, Any]:
        ...
        validation_result = {"is_valid": True, "errors": [], "warnings": [], "score": 1.0}
        ...
        has_required_indicator = any(
            indicator.lower() in case_name.lower() for indicator in self.validation_rules["required_indicators"]
        )
    ...
    def _extract_date_from_case_context(
        self, text: str, case_name: str, citation_start: int, citation_end: int
    ) -> Dict[str, str]:
        """
