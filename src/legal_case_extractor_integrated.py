...
class LegalCaseExtractorIntegrated:
    ...
    def _get_enhanced_reporter_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "supreme_court": ["U\\.S\\.", "S\\.Ct\\.", "L\\.Ed\\.", "L\\.Ed\\.2d"],
            "washington": ["Wn\\.", "Wn\\.2d", "Wn\\.App\\.", "Wash\\.", "Wash\\.2d", "Wash\\.App\\."],
            "pacific": ["P\\.", "P\\.2d", "P\\.3d"],
            "state": [
                "A\\.",
                "A\\.2d",
    ...
    def _deduplicate_extractions(self, extractions: List[CaseExtraction]) -> List[CaseExtraction]:
        ...
        for extraction in sorted_extractions[1:]:
            ...
            for existing in deduplicated:
                if (
                    extraction.start_pos is not None
                    and existing.end_pos is not None
                    and extraction.end_pos is not None
                    and existing.start_pos is not None
                    and extraction.start_pos < existing.end_pos
                    and extraction.end_pos > existing.start_pos
                ):
                    overlaps = True
                    break
