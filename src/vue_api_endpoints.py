...
def process_with_simplified_processor(text, request_id, enable_verification=True):
    ...
    config = ProcessingConfig(
        enable_verification=enable_verification, enable_clustering=True, timeout_seconds=300, cache_results=True
    )
...
def analyze_text():
    ...
    try:
        ...
        if request.files:
            ...
            if not file:
                return jsonify({"error": "No file provided", "message": "Please provide a file to analyze"}), 400
            ...
            if use_simplified:
                ...
                if not text or len(text.strip()) < 10:
                    return (
                        jsonify(
                            {"error": "File extraction failed", "message": "Could not extract readable text from file"}
                        ),
                        400,
                    )
                ...
                result = processor.process_any_input(file, "file", request_id, "api_endpoint", force_mode=force_mode)
                return jsonify(result), 200
        ...
        if input_type == "url" and url:
            ...
            try:
                ...
                extracted_text = citation_service.extract_text_from_input({"type": "url", "url": url})

                if not extracted_text or len(extracted_text.strip()) < 10:
                    logger.warning(
                        f"URL returned insufficient content: {len(extracted_text) if extracted_text else 0} chars"
                    )
                    return (
                        jsonify(
                            {
                ...
                return jsonify({"error": "Failed to fetch URL content", "message": str(e)}), 400
        elif not text:
            return jsonify({"error": "No text provided", "message": "Please provide text to analyze"}), 400
        ...
        if use_simplified:
            ...
            result = processor.process_any_input(text, "text", request_id, "api_endpoint")
...
def get_task_status(task_id):
    ...
    try:
        ...
        # Debug logging
        logger.info(
            f"[Request {task_id}] Job status: {job.get_status()}, is_finished: {job.is_finished}, is_failed: {job.is_failed}"
        )
...
def _get_redis_task_results(task_id):
    ...
    try:
                        ...
                        for message_id, fields in messages:
                            ...
                            if b"return_value" in fields:
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"[SUCCESS] Successfully retrieved task result from Redis stream: {result_stream_key}"
                                    )
                                    return result
                                    ...
                                    try:
                                        ...
                                        logger.info(
                                            f"[SUCCESS] Successfully retrieved task result from Redis stream (JSON): {result_stream_key}"
                                        )
                                        return result
        ...
        keys_to_try = [f"rq:job:{task_id}:result", f"rq:job:{task_id}"]  # Direct result key  # Job hash key
                ...
                if key.endswith(":result"):
                    ...
                    if status:
                        ...
                        if status_str == "failed":
                            ...
                            return {"status": status_str, "task_id": task_id, "message": f"Task is {status_str}"}
...
def get_progress(task_id):
    ...
    try:
        ...
        if progress_data:
            ...
            return jsonify({"success": False, "error": "Task not found or progress not available"}), 404
...
def get_processing_progress():
    ...
    try:
        ...
        return jsonify({"status": "error", "error": str(e), "is_complete": True}), 500
