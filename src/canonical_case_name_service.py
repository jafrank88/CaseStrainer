...
class CanonicalCaseNameService:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "CaseStrainer/2.0 Legal Citation Processor"})
            ...
            try:
                ...
                if result and result.case_name:
                    if not is_valid_case_name(result.case_name):
                        logger.warning(
                            f"[HARDFILTER] Rejected result from {service_name} for '{citation or ''}': '{result.case_name}' is not a valid case name (hard filter)"
                        )
                        continue
                    else:
                        logger.info(
                            f"[HARDFILTER] Accepted result from {service_name} for '{citation or ''}': '{result.case_name}' is a valid case name (hard filter)"
                        )
                if result and result.case_name:
    ...
    def _lookup_courtlistener(self, citation: str) -> Optional[CanonicalResult]:
        ...
        try:
            ...
            params = {"q": query, "type": "o", "format": "json", "page_size": 10, "order_by": "score desc"}  # Opinions
            ...
            for result in results:
                ...
                if (
                    components["volume"] in str(citation_string)
                    and components["page"] in str(citation_string)
                    and components["reporter"] in str(citation_string)
                ):
    ...
    def _lookup_caselaw_access(self, citation: str) -> Optional[CanonicalResult]:
        ...
        try:
            ...
            params = {"cite": citation, "format": "json", "full_case": "false"}
    ...
    def _lookup_westlaw(self, citation: str) -> Optional[CanonicalResult]:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.westlaw_api_key}", "Content-Type": "application/json"}
            params = {"q": citation}
    ...
    def _lookup_web_sources(self, citation: str) -> Optional[CanonicalResult]:
        ...
        try:
            ...
            for res in legal_results[:5]:
                print(
                    f"  Score: {res.get('legal_relevance_score', 0)} | Title: {res.get('title')} | URL: {res.get('url')}"
                )
...
def is_valid_case_name(name: Optional[str]) -> bool:
    ...
    if not re.search(r"\s(v\.|vs\.|v|vs)\s", name, re.IGNORECASE):
        return False
    if re.search(
        r"(\.com|\.net|\.org|https?://|store|watch|eventify|app|allareacodes|youtube|steampowered|outlook)",
        name,
        re.IGNORECASE,
    ):
        return False
...
def get_canonical_case_name_with_date(
    citation: Optional[str], api_key: Optional[str] = None
    ...
    try:
        ...
        normalized_citation = _canonical_service._normalize_citation_comprehensive(
            str(citation_str), purpose="verification"
        )
        ...
        if result and result.case_name is not None:
            if not is_valid_case_name(result.case_name):
                logger.warning(
                    f"[TOP-HARDFILTER] Rejected result for '{citation_str}': '{result.case_name}' is not a valid case name (top-level hard filter)"
                )
                return None
            else:
                return None
                logger.info(
                    f"[TOP-HARDFILTER] Accepted result for '{citation_str}': '{result.case_name}' is a valid case name (top-level hard filter)"
                )
