...
def select_best_cluster_from_result(result, extracted_case_name, debug=False):
    ...
    if best_cluster and best_similarity >= similarity_threshold:
        if debug:
            logger.debug(
                f"Selected best cluster with similarity {best_similarity:.2f} (threshold: {similarity_threshold})"
            )
        return best_cluster
    else:
        if debug:
...
def verify_citations_with_courtlistener_batch(courtlistener_api_key, citations, text):
    ...
    try:
        ...
        headers = {"Authorization": f"Token {courtlistener_api_key}", "Content-Type": "application/json"}
        data = {"text": " ".join(citations_to_verify)}
...
def _verify_with_courtlistener_basic(courtlistener_api_key, citation, extracted_case_name=None):
    ...
    result = {"canonical_name": None, "canonical_date": None, "url": None, "verified": False, "source": None}
                ...
                if found_results:
                    ...
                    if needs_similarity_matching:
                        if len(found_results) > 1:
                            ...
                            cluster = (
                                best_result["clusters"][0] if best_result and best_result.get("clusters") else None
                            )
                        else:
