...
class APIDataPreferenceManager:
    ...
    def __init__(self):
        self.preference_stats = {"total_processed": 0, "api_preferred": 0, "extraction_kept": 0, "improvements_made": 0}
    ...
    def apply_api_preference(self, citations: List[Any]) -> List[Any]:
        ...
        logger.info(
            f"🔄 API Preference applied to {len(citations)} citations: "
    ...
    def _should_prefer_canonical_date(self, extracted_date: Optional[str], canonical_date: str) -> bool:
        ...
        try:
            ...
            if year_diff > 2:
                logger.debug(
                    f"📅 Significant date difference: {extracted_date} vs {canonical_date} ({year_diff} years)"
                )
                return True
