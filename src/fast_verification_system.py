...
class FastVerificationSystem:
    ...
    def __init__(self, enable_web_verification=True, max_timeout=5.0):
        ...
        self.session.headers.update({
            'User-Agent': 'Mozilla/5.0 (compatible; FastVerifier/1.0)'
        })
        ...
        logger.info(f"[FAST-VERIFIER] Initialized with web_verification={enable_web_verification}, timeout={max_timeout}s")
    ...
    def _verify_with_justia_fast(self, citation: str, extracted_case_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_fallback_verification(self, citation: str, extracted_case_name: Optional[str] = None, extracted_date: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'url': None,
            'note': 'Verification based on extracted data' if not actual_case_name else 'Verification with external lookup'
        }
    ...
    def verify_citation_sync(
        ...
        return loop.run_until_complete(
            self.verify_citation(citation, extracted_case_name, extracted_date)
        )
...
class EnhancedFallbackVerifier:
    ...
    def __init__(self, enable_experimental_engines=True):
        ...
        self.fast_verifier = FastVerificationSystem(
            enable_web_verification=enable_experimental_engines,
            max_timeout=5.0
        )
    ...
    async def verify_citation_async(
        ...
        return await self.fast_verifier.verify_citation_async(
            citation, extracted_case_name, extracted_year, timeout
        )
    ...
    def verify_citation_sync(
        ...
        return self.fast_verifier.verify_citation_sync(
            citation, extracted_case_name, extracted_year
        )
    ...
    async def verify_citation(
        ...
        return await self.fast_verifier.verify_citation(
            citation, extracted_case_name, extracted_date, enable_fallback
        )
