...
def group_citations_into_clusters(citations: list, original_text: str | None = None) -> list:
    ...
    for citation in citations:
        ...
        if (
            extracted_name
            and extracted_date
            and extracted_name.strip().upper() != "N/A"
            and extracted_date.strip().upper() != "N/A"
        ):
        ...
        if len(members) > 1:
            ...
            for citation in members:
                ...
                if hasattr(citation, "metadata"):
                    ...
                    cluster_members = [
                        c.citation for c in clusters_by_id[cluster_id] if c.citation != citation.citation
                    ]
                ...
                for cluster_id, cluster_members in clusters_by_id.items():
                    for cluster_member in cluster_members:
                        ...
                        if canonical_name and canonical_date:
                            ...
                            if normalized_extracted == normalized_canonical and extracted_date == canonical_date:
    ...
    for i, c in enumerate(citations):
        if not getattr(c, "extracted_case_name", None) and getattr(c, "extracted_date", None):
            # extracted_case_name should ONLY come from the user's document
    ...
    while i < n:
        ...
        if filtered_case_name:
            ...
            while j < n:
                ...
                next_case_name_match = re.search(
                    r"([A-Z][A-Za-z0-9&.,'\- ]+?)\s*,\s*" + next_citation_escaped, next_context
                )
    ...
    for key, members in fallback_clusters.items():
        ...
        if len(members) > 1:
            ...
            original_names = [
                getattr(c, "extracted_case_name", None) for c in members if getattr(c, "extracted_case_name", None)
            ]
            if original_names:
    ...
    for cluster_id, cluster_citations in clusters_by_id.items():
        try:
            ...
            sorted_citations = sorted(
                cluster_citations,
                key=lambda c: (
                    getattr(c, "start_index", 0) if hasattr(c, "start_index") and c.start_index is not None else 0
                ),
            )
    ...
    for citation_str, group_citations in citation_groups.items():
        if len(group_citations) == 1:
            ...
            cluster_dict = {
                "cluster_id": cluster_id,
                "canonical_name": first_citation.canonical_name,
                "canonical_date": first_citation.canonical_date,
