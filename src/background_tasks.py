...
def reprocess_parallel_citations(batch_size=BATCH_SIZE, sleep_time=SLEEP_BETWEEN, max_batches=None):
    ...
    while True:
        rows = db_manager.execute_query(
            "SELECT citation_text FROM citations WHERE (parallel_citations IS NULL OR parallel_citations = '') LIMIT ?",
            (batch_size,),
        )
        if not rows:
        ...
        try:
            ...
            if current_time - last_backup >= BACKUP_INTERVAL:
                ...
                if is_queue_idle(redis_conn, QUEUE_NAME):
                    ...
                    logger.info("[MAINTENANCE] Queue busy - skipping backup")
            ...
            if current_time - last_cleanup >= CLEANUP_INTERVAL:
                ...
                if is_queue_idle(redis_conn, QUEUE_NAME):
                    ...
                    logger.info("[MAINTENANCE] Queue busy - skipping cleanup")
            ...
            if current_time - last_monitoring >= MONITORING_INTERVAL:
                ...
                if is_queue_idle(redis_conn, QUEUE_NAME):
                    ...
                    logger.info("[MAINTENANCE] Queue busy - skipping monitoring")
