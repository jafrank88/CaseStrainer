...
class UnifiedCitationClusterer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.debug_mode = self.config.get("debug_mode", False) or os.environ.get("DEBUG", "").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    def cluster_citations(
        self, citations: List[Any], original_text: str = "", enable_verification: bool = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _detect_structural_citation_groups(self, sorted_citations: List[Any], text: str) -> List[List[Any]]:
        ...
        for pattern_idx, pattern in enumerate(citation_structure_patterns):
            ...
            for match in matches:
                ...
                logger.info(
                    f"🎯 STRUCTURAL: Found pattern {pattern_idx} - Case: '{case_name}', Citations: '{citations_text}', Year: {year}"
                )
                ...
                for citation in sorted_citations:
                    ...
                    if citation_start >= match_start and citation_end <= match_end and citation_text not in visited:
                ...
                if pattern_citations:
                    ...
                    logger.info(
                        f"🔗 STRUCTURAL: Created group for '{case_name}' with {len(pattern_citations)} citations"
                    )
    ...
    def _find_nearby_parallel_citations(
        self, citation: Any, sorted_citations: List[Any], text: str, visited: Set[str]
    ) -> List[Any]:
        """
    ...
    def _extract_and_propagate_metadata(self, groups: List[List[Any]], text: str) -> Dict[str, List[Any]]:
        ...
        for group in groups:
            ...
            first_citation_text = (
                sorted_group[0].get("citation")
                if isinstance(sorted_group[0], dict)
                else getattr(sorted_group[0], "citation", "unknown")
            )
            ...
            if len(sorted_group) > 1:
                ...
                if not year or year == "N/A":
                    ...
                    if candidate_years:
                        ...
                        logger.warning(
                            f"📊 METADATA: Selected most common year '{year}' from {len(candidate_years)} candidates"
                        )
            else:
                year = self._extract_year_from_citation(sorted_group[0], text)

            preferred_year = prefer_canonical_year(year, {"canonical_year": canonical_year} if canonical_year else {})
            ...
            if not extracted_case_name or extracted_case_name == "N/A":
                for citation in sorted_group:
                    ...
                    if extracted_name and extracted_name != "N/A" and len(extracted_name) > 5:
                        ...
                        logger.warning(
                            f"🔄 METADATA: Using fallback case name '{extracted_case_name}' from citation {sorted_group.index(citation)}"
                        )
                        break
            ...
            if not preferred_year or preferred_year == "N/A":
                for citation in reversed(sorted_group):
                    ...
                    if extracted_year and extracted_year != "N/A" and extracted_year.isdigit():
                        ...
                        logger.warning(
                            f"🔄 METADATA: Using fallback year '{preferred_year}' from citation {sorted_group.index(citation)}"
                        )
                        break

            # IMPROVED: Validate extracted data before propagation
            if preferred_case_name and preferred_case_name != "N/A" and preferred_year and preferred_year != "N/A":
                logger.warning(
                    f"✅ METADATA: Validated extraction - Case: '{preferred_case_name}', Year: '{preferred_year}'"
                )
            else:
                logger.warning(
                    f"⚠️ METADATA: Incomplete extraction - Case: '{preferred_case_name}', Year: '{preferred_year}'"
                )
    ...
    def _merge_clusters_by_extracted_metadata(self, clusters: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
            ...
            if len(reporter_types) > 1:
                ...
                if sample_reporter == "wl":
                    ...
                    metadata_cluster_key = create_cluster_key_from_extracted_data(
                        sample_name, sample_year, sample_citation_text
                    )
                else:
        ...
        logger.info(
            f"Merged {len(clusters)} initial clusters into {len(merged_clusters)} final clusters (preserving parallel relationships)"
        )
    ...
    def _extract_case_name_from_citation(self, citation: Any, text: str) -> str:
        ...
        if extracted_name and extracted_name != "N/A":
            ...
            if is_valid_case_name(extracted_name):
                ...
                logger.warning(
                    f"[CLUSTERING-REJECT] Enhanced extraction returned invalid name '{extracted_name}' for {citation_text}"
                )
                extracted_name = None
        ...
        if extracted_name and extracted_name != "N/A":
            ...
            if not is_valid_case_name(extracted_name):
                logger.warning(
                    f"[CLUSTERING-REJECT] Fallback extraction returned invalid name '{extracted_name}' for {citation_text}"
                )
                extracted_name = None
    ...
    def _extract_case_name_enhanced(
        self, citation: Any, text: str, start_index: int, end_index: int, citation_text: str
    ) -> str:
        """
        ...
        result = extract_case_name_and_date_unified_master(
            text=text, citation=citation_text, start_index=start_index, end_index=end_index, debug=False
        )
        ...
        if extracted_name and extracted_name != "N/A":
            ...
            if not is_valid_case_name(extracted_name):
                logger.warning(
                    f"[CLUSTERING-REJECT] Master extraction returned invalid name '{extracted_name}' for {citation_text}"
                )
                return "N/A"
    ...
    def _clean_extracted_case_name(self, case_name: str) -> str:
        ...
        contamination_phrases = [
            ...
            r"\b(reviews?\s+de\s+novo)\b",
            r"\b(we\s+hold)\b",
            r"\b(we\s+conclude)\b",
            r"\b(we\s+find)\b",
            r"\b(the\s+legislature)\b",
            r"\b(statutory\s+language)\b",
            r"\b(plain\s+meaning)\b",
            # Citation and reference phrases
            r"\b(see|citing|quoting|accord|id\.|ibid\.)\b",
            r"\b(brief\s+at|opening\s+br\.|reply\s+br\.)\b",
            r"\b(internal\s+quotation\s+marks)\b",
            r"\b(alteration\s+in\s+original)\b",
            r"\b(may\s+ask\s+this\s+court)\b",
            r"\b(resolution\s+of\s+that\s+question)\b",
            r"\b(necessary\s+to\s+resolve)\b",
            r"\b(case\s+before\s+the)\b",
            # NEW: Procedural and analysis phrases that contaminate case names
            r"\b(we\s+accepted\s+certification)\b",
            r"\b(analysis\s+are\s+that)\b",
            r"\b(and\s+by\s+the)\b",
            r"\b(both\s+the\s+defendant\s+and)\b",
            r"\b(argue\s+that)\b",
            r"\b(job\s+applicants\s+who)\b",
            r"\b(do\s+not\s+have\s+a\s+good\s+faith)\b",
            r"\b(intent\s+to\s+obtain\s+employment)\b",
            r"\b(lack\s+statutory\s+standing)\b",
            r"\b(because\s+they\s+are\s+not)\b",
            r"\b(within\s+the\s+zone\s+of\s+interest)\b",
            r"\b(protected\s+by\s+the\s+statute)\b",
            r"\b(have\s+not\s+suffered\s+an\s+injury)\b",
            r"\b(in\s+fact)\b",
            r"\b(economic\s+or\s+otherwise)\b",
            r"\b(we\s+decline\s+to\s+address)\b",
            r"\b(the\s+standing\s+argument)\b",
            r"\b(as\s+it\s+is\s+beyond\s+the\s+scope)\b",
            r"\b(of\s+the)\b",
            # Court procedural language
            r"\b(the\s+court\s+held)\b",
            r"\b(the\s+court\s+found)\b",
            r"\b(the\s+court\s+ruled)\b",
            r"\b(petitioner\s+argues)\b",
            r"\b(respondent\s+contends)\b",
            r"\b(plaintiff\s+claims)\b",
            r"\b(defendant\s+maintains)\b",
            r"\b(appellant\s+asserts)\b",
            r"\b(appellee\s+responds)\b",
        ]
        ...
        if cleaned and len(cleaned) > 3:
            ...
            if word_count >= 2:  # Lower threshold - Too many legal procedural words
                logger.warning(
                    f"🚫 CONTAMINATION: Rejected case name '{cleaned}' - contains {word_count} legal procedural words"
                )
                return "N/A"
    ...
    def _extract_year_enhanced(self, citation: Any, text: str) -> str:
        ...
        if best_year:
            logger.warning(
                f"🎯 YEAR_EXTRACT: Selected year '{best_year}' for citation '{getattr(citation, 'citation', 'unknown')}'"
            )
            return best_year
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for result in response_data:
                        ...
                        if citation_text and result.get("clusters"):
                            ...
                            if (
                                ...
                                and case_name.strip()
                                and absolute_url
                                and absolute_url.strip()
                                and len(case_name.strip()) >= 5
                            ):
                        ...
                        if result.get("verified", False):
                            ...
                            if extracted_name and extracted_name != "N/A" and canonical_name:
                                ...
                                if not names_compatible:
                                    logger.warning(
                                        f"⚠️  Name mismatch: {citation.citation} - keeping both extracted and canonical"
                                    )
                            ...
                            if validation_passed and result_source in trusted_api_sources:
                                ...
                                if (
                                    getattr(citation, "extracted_case_name", None) == "N/A"
                                    or not getattr(citation, "extracted_case_name", None)
                                ) and canonical_name:
                                    pass
                                ...
                                logger.warning(
                                    f"🚫 REJECTED canonical data from untrusted source '{result_source}' for {citation.citation}"
                                )
                    ...
                    logger.warning(
                        f"Batch verification failed with status {response.status_code}: {response.text[:200]}"
                    )
                    for citation in batch:
    ...
    def _apply_comprehensive_verification(self, citations: List[Any]):
        ...
        try:
            logger.info(
                f"Applying unified verification with possible match support to {len(citations)} unverified citations"
            )
            ...
            for citation in citations:
                try:
                    ...
                    has_verification_data = (
                        hasattr(citation, "verification_result")
                        and citation.verification_result
                        and citation.verification_result.get("verified", False)
                    )

                    if has_verification_data:
                        logger.info(
                            f"Skipping fallback verification for {citation_text} - already verified via enhanced verification"
                        )
                        continue
                        ...
                        if loop.is_running():
                            ...
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(
                                    asyncio.run,
                                    verifier.verify_citation(citation_text, extracted_case_name, extracted_date),
                                )
                    ...
                    if result and (result.verified or getattr(result, "possible_match", False)):
                        ...
                        logger.warning(
                            f"VERIFICATION_FIX: Set verified={result.verified}, possible_match={getattr(result, 'possible_match', False)} for citation '{citation.citation}'"
                        )
                        ...
                        logger.info(
                            f"✓ Unified verification: {citation_text} -> {result.canonical_name or 'N/A'} (via {result.source}) - verified={result.verified}, possible_match={getattr(result, 'possible_match', False)}"
                        )
                    else:
        ...
        try:
            ...
            if case_name and case_name != "N/A":
                ...
                if is_valid_case_name(case_name):
                    logger.warning(
                        f"✅ Master extraction found: '{case_name}' for citation: '{getattr(citation, 'citation', 'unknown')}'"
                    )
                    return case_name
                else:
                    logger.warning(
                        f"🚫 Master extraction returned invalid name '{case_name}' for citation: '{getattr(citation, 'citation', 'unknown')}'"
                    )
        except Exception as e:
                ...
                if len(case_name) > 5 and case_name != "N/A":
                    ...
                    if cleaned_name:
                        ...
                        if is_valid_case_name(cleaned_name):
                            ...
                            logger.warning(
                                f"🚫 Pattern extraction returned invalid name '{cleaned_name}' for citation: '{getattr(citation, 'citation', 'unknown')}'"
                            )
    ...
    def _apply_core_clustering_logic(self, citations: List[Any]) -> Dict[str, List[Any]]:
        ...
        for group in parallel_groups:
            ...
            sorted_group = sorted(
                group, key=lambda c: c.start_index if hasattr(c, "start_index") and c.start_index is not None else 0
            )
        ...
        for citation in citations:
            ...
            for parallel_cite in parallel_citations:
                if parallel_cite in citation_lookup and parallel_cite not in visited:
                    ...
                    if (
                        not citation_extracted
                        or not parallel_extracted
                        or citation_extracted == "N/A"
                        or parallel_extracted == "N/A"
                    ):
                        logger.error(
                            f"🚫 [FIX #58B] Rejecting - missing extracted names: '{citation_extracted}' vs '{parallel_extracted}'"
                        )
                    ...
                    if not citation_year or not parallel_year or citation_year == "N/A" or parallel_year == "N/A":
                        logger.error(
                            f"🚫 [FIX #58B] Rejecting - missing extracted years: '{citation_year}' vs '{parallel_year}'"
                        )
                    ...
                    if not self._are_case_names_compatible(citation_extracted, parallel_extracted):
                        logger.error(
                            f"🚫 [FIX #58B] Rejecting - incompatible extracted names: '{citation_extracted}' vs '{parallel_extracted}'"
                        )
                        continue  # Different cases - don't group

                    logger.error(
                        f"✅ [FIX #58B] ACCEPTING parallel: '{citation_extracted}' ({citation_year}) + '{parallel_extracted}' ({parallel_year})"
                    )
                    ...
                    if citation.citation not in reverse_parallels:
                        logger.warning(
                            f"🚫 [FIX #58B] Skipping non-bidirectional parallel: {citation.citation} -> {parallel_cite}"
                        )
    ...
    def _format_clusters_for_output(self, clusters: Dict[str, List[Any]]) -> List[Dict[str, Any]]:
        ...
        for cluster_id, citations in clusters.items():
            ...
            verified_citations = [
                c for c in citations if getattr(c, "verified", False) or getattr(c, "is_verified", False)
            ]
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"CLUSTER_DEBUG: Processing cluster {cluster_id} with {len(citations)} citations, {len(verified_citations)} verified"
            )
            ...
            if year and year != "N/A":
                if re.match(r"^\d{4}$", str(year)):
                    ...
                    logger.warning(
                        f"[CONTAMINATION-FIX] Extracted year '{display_year}' from contaminated date '{year}' for cluster {cluster_id}"
                    )
                else:
                    ...
                    if year_match:
                        ...
                        logger.warning(
                            f"[CONTAMINATION-FIX] Extracted year '{display_year}' from malformed date '{year}' for cluster {cluster_id}"
                        )
                    else:
                        display_year = "Unknown Year"
                        logger.warning(
                            f"[CONTAMINATION-ERROR] Could not extract year from '{year}' for cluster {cluster_id}"
                        )
            else:
                display_year = "Unknown Year"
                ...
                for sc in serialized_citations:
                    if isinstance(sc, dict):
                        ...
                        if old_date and old_date != display_year and old_date != "N/A":
                            logger.debug(
                                f"[CLUSTER-DATE-FIX] Updated citation extracted_date: {old_date} -> {display_year}"
                            )
            ...
            if cluster_integrity_warnings:
                ...
                logger.warning(
                    f"⚠️ CLUSTER_INTEGRITY: Cluster {cluster_id} has integrity issues: {cluster_integrity_warnings}"
                )
            ...
            for citation in citations:
                ...
                if isinstance(citation, dict):
                    ...
                    citation_text = getattr(citation, "citation", "unknown")
                logger.warning(
                    f"CLUSTERING_FIX: Set cluster_case_name='{final_cluster_case_name}' for citation '{citation_text}'"
                )
                ...
                if any_verified and not getattr(citation, "verified", False) and best_verified_citation:
                    ...
                    logger.info(
                        f"✓ Propagated verification from {best_verified_citation.citation} to {citation.citation} (true_by_parallel)"
                    )
    ...
    def _validate_cluster_integrity(
        self, citations: List[Any], expected_case_name: str, expected_year: str
    ) -> List[str]:
        """
        ...
        for citation in citations:
            ...
            if isinstance(citation, dict):
                ...
                citation_case_name = getattr(citation, "extracted_case_name", None) or getattr(
                    citation, "canonical_name", None
                )
            ...
            if citation_case_name and citation_case_name != "N/A" and citation_case_name != expected_case_name:
                ...
                if not (citation_case_name in expected_case_name or expected_case_name in citation_case_name):
                    warnings.append(
                        f"Inconsistent case name: '{citation_case_name}' vs expected '{expected_case_name}'"
                    )
        ...
        for cluster_id, citations in clusters.items():
            ...
            for citation in citations:
                if not getattr(citation, "verified", False):
                    ...
                    citation.verification_source = getattr(
                        best_verified_citation,
                        "verification_source",
                        getattr(best_verified_citation, "source", "true_by_parallel"),
    ...
    def _serialize_citation_object(self, citation: Any, cluster_citation_texts: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"citation": str(citation), "error": f"Serialization error: {str(e)}"}
...
def cluster_citations_unified(
    citations: List[Any], original_text: str = "", enable_verification: bool = None
) -> List[Dict[str, Any]]:
    """
    DEPRECATED: Use cluster_citations_unified_master() instead.
    ...
    return cluster_citations_unified_master(
        citations=citations, original_text=original_text, enable_verification=enable_verification
    )
