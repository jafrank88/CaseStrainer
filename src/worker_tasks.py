...
def simple_task():
    ...
    return {"status": "success", "message": "Hello from worker! This is a test task.", "python_path": __file__}
...
def test_imports() -> Dict[str, Dict[str, Any]]:
    ...
    for module_name in modules:
        try:
            ...
            results[module_name] = {"status": "success", "path": getattr(module, "__file__", "unknown")}
        except Exception as e:
            results[module_name] = {"status": "error", "error": str(e)}

    # Also include sys.path for debugging
    results["_sys_path"] = {"status": "info", "path": sys.path}
...
def verify_citations_enhanced(citations, text, doc_id, doc_type, metadata):
    ...
    try:
            ...
            try:
                ...
                for i, citation in enumerate(citations):
                    try:
                        ...
                        verified_citations.append(
                            {"id": i + 1, "original_citation": citation, "status": "error", "error": str(e)}
                        )
                ...
                if verified_citations:
                    result["status"] = (
                        "partial" if any(c["status"] != "verified" for c in verified_citations) else "success"
                    )
        ...
        return {"status": "error", "doc_id": doc_id, "error": str(e), "timestamp": datetime.utcnow().isoformat()}
