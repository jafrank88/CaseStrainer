...
class BluebookFormatter:
    ...
    def _fix_citation_spacing(self, citation: str) -> str:
        ...
        citation = re.sub(
            r"F\.\s*Supp\.\s*(\d+d)?", lambda m: f"F. Supp.{' ' + m.group(1) if m.group(1) else ''}", citation
        )

        citation = re.sub(
            r"L\.\s*Ed\.\s*(\d+d)?", lambda m: f"L. Ed.{' ' + m.group(1) if m.group(1) else ''}", citation
        )
    ...
    def extract_citation_components(self, citation: str) -> Dict[str, Optional[str]]:
        ...
        """
        components: Dict[str, Optional[str]] = {"volume": None, "reporter": None, "page": None, "pinpoint": None}
...
def format_citation_bluebook(
    case_name: str, citation: str, year: Optional[str] = None, court: Optional[str] = None
) -> str:
    """
