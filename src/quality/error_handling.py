...
class APIError(CaseStrainerError):
    ...
    def __init__(
        self, message: str, api_name: str, status_code: Optional[int] = None, response_data: Optional[Any] = None
    ):
        ...
        self.details.update(
            {"api_name": self.api_name, "status_code": self.status_code, "response_data": self.response_data}
        )
...
class ErrorTracker:
    ...
    def record_error(
        self, error: Exception, context: Optional[ErrorContext] = None, severity: ErrorSeverity = ErrorSeverity.MEDIUM
    ):
    ...
    def _summarize_input(self, input_data: Any) -> Dict[str, Any]:
        ...
        if isinstance(input_data, str):
            ...
            return {"type": "dict", "keys": list(input_data.keys())[:10]}
        else:
            return {"type": type(input_data).__name__, "value": str(input_data)[:100]}
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logging.warning(
                        f"Attempt {attempt + 1} failed for {operation}: {e}. Retrying in {current_delay}s..."
                    )
                    time.sleep(current_delay)
                    current_delay *= backoff_factor
                except Exception as e:
                    context = ErrorContext(
                        operation=f"{operation}_non_retryable", additional_context={"attempt": attempt + 1}
                    )
                    error_tracker.record_error(e, context, ErrorSeverity.HIGH)
                    raise
            if last_exception is not None:
...
class HealthChecker:
    ...
    def register_check(self, name: str, check_func: Callable[[], bool], interval: float = 60.0):
        ...
        self.checks[name] = {"func": check_func, "interval": interval, "last_result": None, "last_error": None}
    ...
    def run_checks(self) -> Dict[str, Any]:
        ...
        for name, check_info in self.checks.items():
            ...
            if current_time - last_check < check_info["interval"]:
                results[name] = {"status": "cached", "result": check_info["last_result"], "last_check": last_check}
                continue
            ...
            try:
                ...
                results[name] = {"status": "error", "error": str(e), "last_check": current_time}
