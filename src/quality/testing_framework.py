...
class TestDataGenerator:
        ...
        for i in range(citation_count):
            if i < len(self.landmark_cases):
                ...
                if include_parallels and case_name == "Gideon v. Wainwright":
                    citations.extend(["Gideon v. Wainwright, 83 S. Ct. 792 (1963)", "9 L. Ed. 2d 799 (1963)"])
    ...
    def generate_large_document(self, size_kb: int = 50) -> str:
        ...
        while len(current_text) < target_size:
            ...
            additional_text += base_text.replace(
                "This is a legal document", f"This is section {len(current_text) // 1000} of the legal document"
            )
            ...
            if "verifier" in services and size_kb <= 10:  # Only test small documents
                ...
                if test_citations:
                    ...
                    size_results["verification"] = {
                        ...
                        "throughput_citations_per_sec": (
                            len(test_citations) / verification_time if verification_time > 0 else 0
                        ),
                    }
