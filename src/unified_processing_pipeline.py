...
class UnifiedProcessingPipeline:
    ...
    async def process_citations(self, 
                               text: str, 
        ...
        logger.info(f"[PIPELINE-{context.trace_id}] DEBUG: Creating processor with enable_verification={enable_verification}")
        logger.info(f"[PIPELINE-{context.trace_id}] DEBUG: Processor config.enable_verification = {config.enable_verification}")
    ...
    async def _verify_citations(self, citations: List[CitationResult], text: str, context: ProcessingContext) -> List[CitationResult]:
        ...
        try:
            ...
            logger.info(f"[PIPELINE-{context.trace_id}] Progressive verification timeout: {timeout_seconds}s for {len(citations)} citations")
            ...
            try:
                ...
                logger.info(f"[PIPELINE-{context.trace_id}] Verification completed, returned {len(verified_citations)} citations")
                return verified_citations
            except asyncio.TimeoutError:
                logger.warning(f"[PIPELINE-{context.trace_id}] Verification timed out after {timeout_seconds}s, returning original citations")
    ...
    async def _apply_parallel_verification(self, citations: List[CitationResult], context: ProcessingContext) -> List[CitationResult]:
        ...
        try:
            ...
            logger.info(f"[PIPELINE-{context.trace_id}] Parallel verification completed - {parallel_count} citations marked")
    ...
    def _is_document_case_contamination_post_process(self, extracted_name: str, document_primary_case_name: str) -> bool:
        """
    ...
    async def _format_response(self, citations: List[CitationResult], context: ProcessingContext) -> Dict[str, Any]:
        ...
        try:
            ...
            for cit in citations:
                ...
                if 'extracted_case_name' in cit_dict:
                    ...
                    if (cleaned_name == "N/A" or not cleaned_name) and canonical_name and canonical_name != "N/A":
                        logger.info(f"[FORMAT-RESPONSE] Using canonical_name '{canonical_name}' as extracted_case_name (extraction returned N/A)")
                    ...
                    if cleaned_name and cleaned_name != "N/A":
                        ...
                        has_et_al = 'ET AL' in cleaned_name_upper or 'ETAL' in cleaned_name_upper.replace(' ', '').replace('.', '').replace(',', '')
                        has_role_word = any(role in cleaned_name_upper for role in ['PETITIONER', 'RESPONDENT', 'APPELLANT', 'APPELLEE', 'PLAINTIFF', 'DEFENDANT'])
                        has_no = 'NO.' in cleaned_name_upper or ' NO ' in cleaned_name_upper or cleaned_name_upper.endswith(' NO')
                        header_pattern_match = re.search(r'ET\s+AL\.?\s*,?\s*(?:PETITIONER|RESPONDENT|APPELLANT|APPELLEE|PLAINTIFF|DEFENDANT)', cleaned_name_upper)
                        
                        if (has_et_al and has_role_word) or (has_role_word and has_no) or header_pattern_match:
                            logger.error(f"[FORMAT-RESPONSE] FINAL REJECTION: Header pattern detected in '{cleaned_name}' for citation '{cit_dict.get('citation', 'unknown')}' - setting to N/A")
                            cleaned_name = "N/A"
                    
                    # USER FIX: Reject fragment extractions like "Inc v. Montgomery"
                    if cleaned_name and cleaned_name != "N/A":
                        fragment_match = re.match(r'^(Inc\.?|Corp\.?|LLC|L\.L\.C\.|Ltd\.?|Co\.?|Ass\'?n|Assoc\.?|Org\.?)\s+v\.?\s+', cleaned_name, re.IGNORECASE)
                        if fragment_match:
                            logger.error(f"[FORMAT-RESPONSE] FRAGMENT REJECTION: '{cleaned_name}' starts with company suffix - using canonical or N/A")
                    ...
                    if cleaned_name and cleaned_name != "N/A" and document_primary_case_name:
                        is_contaminated = self._is_document_case_contamination_post_process(cleaned_name, document_primary_case_name)
                        if is_contaminated:
                            logger.error(f"[POST-PROCESS-CONTAMINATION] ❌ REJECTING contaminated name '{cleaned_name}' for citation '{cit_dict.get('citation', 'unknown')}' (matches document primary '{document_primary_case_name}')")
                            logger.info(f"[POST-PROCESS-CONTAMINATION] Setting to N/A (canonical_name='{canonical_name}' available but not used per data separation rule)")
                            cleaned_name = "N/A"
                        else:
                            logger.debug(f"[POST-PROCESS-CONTAMINATION] ✓ Keeping name '{cleaned_name}' (does not match primary '{document_primary_case_name}')")
                    ...
                    if cleaned_name and cleaned_name != "N/A" and canonical_name and canonical_name != "N/A":
                        if cleaned_name.lower() != canonical_name.lower():
                            ...
                            logger.info(f"[DATA-SEPARATION] Extracted '{cleaned_name}' differs from canonical '{canonical_name}' - keeping both separate")
            ...
            for i, cit_dict in enumerate(citation_dicts):
                ...
                if cit_pos and cit_pos > 10:
                    ...
                    if appellate_history_type and previous_case_name and previous_case_name != 'N/A':
                        ...
                        logger.info(f"[APPELLATE-HISTORY] Citation {cit_dict.get('citation')} is {appellate_history_type} of '{previous_case_name}'")
            ...
            try:
                ...
                logger.info(f"[PIPELINE-{context.trace_id}] Annotated mismatch flags for {len(citation_dicts)} citations")
            except Exception as e:
            ...
            try:
                from src.unified_clustering_master_optimized import cluster_citations_optimized
                if cluster_citations_unified_master is None:
                    ...
                    logger.error(f"[CLUSTERING-TRACE] Calling cluster_citations_unified_master with {len(citation_dicts)} citations")
                    clusters = cluster_citations_unified_master(
                        citation_dicts, 
                        original_text=context.input_text, 
                        enable_verification=False  # Already verified
                    )
                    if not clusters:
                        logger.error(f"[CLUSTERING-TRACE] cluster_citations_unified_master returned empty, using fallback")
                        clustering_source = "fallback_parallel_citations"
                        clusters = self._create_clusters_from_parallel_citations(citation_dicts)
                    else:
                        clustering_source = "unified_master"
                        logger.error(f"[CLUSTERING-TRACE] cluster_citations_unified_master returned {len(clusters)} clusters")
            except Exception as e:
            ...
            try:
                ...
                logger.info(f"[PIPELINE-{context.trace_id}] Updated cluster-level mismatch flags for {len(clusters)} clusters")
            except Exception as e:
                logger.warning(f"[PIPELINE-{context.trace_id}] Failed to update cluster mismatch flags: {e}", exc_info=True)
    ...
    def _create_clusters_from_parallel_citations(self, citation_dicts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def _extract_year_from_citation(citation_text):
            ...
            year_in_format_match = re.match(r'^(\d{4})\s+(?:WY|ND|OK|SD|UT|WI|MT|AL|AK|AR|AZ|CA|CO|CT|DE|FL|GA|HI|ID|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|NE|NV|NH|NJ|NM|NY|NC|OH|OR|PA|RI|SC|TN|TX|VT|VA|WA|WV|DC)\s+\d+', citation_text, re.IGNORECASE)
        ...
        for cluster_id, citations in clusters_dict.items():
            ...
            if not best_extracted_name:
                best_extracted_name = primary_citation.get('submitted_display_name') or primary_citation.get('extracted_case_name')
            ...
            if primary_citation.get('verified', False):
                cluster_case_name = primary_citation.get('canonical_name') or primary_citation.get('extracted_case_name')
            ...
            cluster = {
                ...
                'submitted_display_date': primary_citation.get('submitted_display_date') or primary_citation.get('extracted_date'),
                # Mismatch flags
...
async def process_citations_unified(text: str, 
                                 processing_mode: str = "enhanced_sync",
                                 enable_parallel_verification: bool = True,
                                 enable_verification: bool = False,
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        for cit in result.get('citations', []):
            print(f"  {cit.get('citation')}: verified={cit.get('verified')}, true_by_parallel={cit.get('true_by_parallel')}")
