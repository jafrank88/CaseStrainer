...
class UnifiedCaseNameExtractor:
            ...
            if citation:
                ...
                if volume_case_name:
                    ...
                    if vol_citation_start != -1 and vol_citation_end is not None:
                        date, year = self._extract_date_from_case_context(
                            text, volume_case_name, vol_citation_start, vol_citation_end
                        )
    ...
    def _extract_case_name_unified(
        self, context: str, citation: Optional[str] = None
        ...
        has_case_indicators = any(pattern.search(context) for pattern in self._common_patterns_compiled.values())
            ...
            try:
                ...
                try:
                    ...
                    if match_time > 0.1:  # Log slow matches
                        self.logger.debug(f"Slow pattern match: {pattern_name} took {match_time:.3f}s")
    ...
    def _clean_case_name_unified(self, case_name: str, context: str) -> Optional[str]:
        ...
        sentence_split = re.search(
            r"[.!?]\s*([A-Z][^.!?]*(?:v\.|vs\.|versus|In re|Estate of|Matter of)[^.!?]*)", case_name
        )
        if sentence_split:
    ...
    def _validate_case_name_unified(self, case_name: str) -> bool:
        ...
        if not any(indicator in case_lower for indicator in ["v.", "vs.", "versus", "in re", "estate of", "matter of"]):
            return False
    ...
    def _extract_date_unified(
        self, text: str, citation_start: int, citation_end: int
    ...
    def extract_case_name_from_citation_volume(self, text: str, citation: str) -> Optional[str]:
        ...
        try:
            ...
            if comma_pos != -1:
                ...
                # No comma found, use end of text
                case_name_end = len(text)
...
def extract_case_name_and_date_unified(
    text: str, citation: Optional[str] = None, citation_start: Optional[int] = None, citation_end: Optional[int] = None
) -> Dict[str, Any]:
    """
...
def extract_case_name_only_unified(
    text: str, citation: Optional[str] = None, citation_start: Optional[int] = None, citation_end: Optional[int] = None
) -> str:
