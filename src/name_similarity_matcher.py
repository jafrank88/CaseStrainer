...
def normalize_case_name(name: str) -> str:
    ...
    name = re.sub(
        r"\b(inc|corp|ltd|llc|co|assoc|bros|dr|jr|sr|st|mt|ft|univ|nat\'l|fed|comm\'n|bd|ctr|dept|hosp)\b\.?", "", name
    )
...
def calculate_case_name_similarity(name1: str, name2: str) -> float:
    ...
    combined_similarity = 0.4 * seq_similarity + 0.4 * word_similarity + 0.2 * substring_similarity

    return combined_similarity


def select_best_courtlistener_result(
    results: List[Dict[str, Any]], extracted_case_name: str, debug: bool = False
    ...
    for i, result in enumerate(results):
        ...
        if not case_name:
            case_name = (
                result.get("canonical_name") or result.get("case_name") or result.get("caseName") or result.get("name")
            )

        if not case_name:
            if debug:
                print(f"Skipping empty case_name at index {i}")
            continue
        ...
        if similarity > 0.7:  # Threshold for considering it a match
            matches.append({"index": i, "case_name": case_name, "similarity": similarity})
...
def test_name_similarity():
    ...
    best_result = select_best_courtlistener_result(
        mock_results, extracted_name, debug=os.getenv("FLASK_DEBUG", "False").lower() == "true"
    )
    if best_result:
