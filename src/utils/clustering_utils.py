...
def create_cluster_key_from_extracted_data(
    extracted_case_name: Optional[str], extracted_date: Optional[str], citation_text: str = ""
) -> str:
    """
    ...
    if extracted_date and extracted_date not in ("N/A", "Unknown", "Unknown Year"):
        ...
        if re.match(r"^\d{4}-\d{2}-\d{2}", str(extracted_date)):
            ...
            if year_match:
                ...
                logger.warning(f"[CLUSTER-KEY] Extracted year '{clean_year}' from malformed date '{extracted_date}'")
...
def validate_cluster_key(cluster_key: str, source: str = "unknown") -> bool:
    ...
    if re.search(r"\d{4}-\d{2}-\d{2}", cluster_key):
        logger.error(f"[CLUSTER-KEY-CONTAMINATION] Cluster key from {source} contains ISO date: '{cluster_key}'")
        return False

    # Check for "canonical" prefix
    if cluster_key.startswith("canonical_"):
        logger.error(f"[CLUSTER-KEY-CONTAMINATION] Cluster key from {source} uses canonical prefix: '{cluster_key}'")
        return False
