...
def _filter_headers_and_footnotes_from_context(context: str) -> str:
    ...
    for line in lines:
        ...
        if re.search(r'\bNo\.?\s*\d+[-\.]\d+', line_stripped, re.IGNORECASE):
            ...
            if (line_stripped.isupper() and len(line_stripped) > 20) or \
               re.search(r'\b(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\b', line_stripped, re.IGNORECASE):
                logger.debug(f"[HEADER-FILTER] Filtering header line: '{line_stripped[:50]}...'")
                continue
        
        # Skip lines containing header patterns
        if re.search(r'ET\s+AL\.?\s*,?\s*(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\b', line_stripped, re.IGNORECASE):
            logger.debug(f"[HEADER-FILTER] Filtering line with ET AL pattern: '{line_stripped[:50]}...'")
            continue
        
        if re.search(r'\b(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\s*[,\.]\s*NO\.?\b', line_stripped, re.IGNORECASE):
        ...
        if re.search(r'\b(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\s*[,\.]\s*NO\.?\s*$', line_stripped, re.IGNORECASE):
        ...
        if re.search(r'[Pp]age\s+\d+', line_stripped) or re.search(r'^\d+\s*$', line_stripped):
            ...
            if len(line_stripped) < 50 or re.search(r'\bv\.\s+', line_stripped, re.IGNORECASE):
                ...
                if re.search(r'ET\s+AL|(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\s*[,\.]\s*NO', line_stripped, re.IGNORECASE):
        ...
        if line_stripped.isupper() and len(line_stripped) > 15:
            ...
            if re.search(r'ET\s+AL|(?:Petitioners?|Appellants?|Plaintiffs?|Appellees?|Respondents?)\s*[,\.]\s*NO|No\.\s*\d+', line_stripped, re.IGNORECASE):
                # If it's near the end of context (last 3 lines), likely a footer
                line_index = len(filtered_lines)
                total_lines = len(lines)
                if line_index >= total_lines - 3:
                    logger.debug(f"[FOOTER-FILTER] Filtering all-caps footer line (near end): '{line_stripped[:50]}...'")
                    continue
...
def _add_missing_words(case_name: str, context: str) -> str:
    ...
    cities = ["Bellevue", "Seattle", "Spokane", "Tacoma", "Vancouver", "Portland", "Snohomish", "King", "Pierce", "Thurston"]
...
def get_adaptive_context_for_citation(
    ...
    return get_strict_context_for_citation(
        text, citation_start, citation_end, all_citation_positions, max_lookback
    )
    ...
    if strict_context:
        ...
        for dash in ('—', '–', '--'):
            ...
            if last_dash != -1:
                ...
                is_part_of_name = any(s in after_dash for s in ['llc', 'inc', 'corp', 'ltd', 'co.', ' tv ', 'radio', 'broadcast'])
                if is_part_of_name:
...
def extract_case_name_from_strict_context(
    ...
    logger.error(f"[DEBUG] [STRICT-CONTEXT-ISOLATOR-CALLED] extract_case_name_from_strict_context() invoked for {citation_text}")
    ...
    for pattern_idx, pattern in enumerate(patterns, 1):
        try:
            ...
            if matches:
                logger.error(f"[PATTERN-DEBUG] Pattern {pattern_idx} found {len(matches)} matches in context: '{context}'")
            ...
            for match in matches:
                ...
                has_et_al_match = 'ET AL' in match_text or 'ETAL' in match_text.replace(' ', '').replace('.', '').replace(',', '')
                has_role_word_match = any(re.search(role, match_text) for role in [r'PETITIONER', r'RESPONDENT', r'APPELLANT', r'APPELLEE', r'PLAINTIFF', r'DEFENDANT'])
                has_no_match = 'NO.' in match_text or ' NO ' in match_text or match_text.endswith(' NO') or re.search(r'\bNO\.?\s*\d+', match_text)
                header_pattern_match = re.search(r'ET\s+AL\.?\s*,?\s*(?:PETITIONER|RESPONDENT|APPELLANT|APPELLEE|PLAINTIFF|DEFENDANT)', match_text)
                
                if (has_et_al_match and has_role_word_match) or (has_role_word_match and has_no_match) or header_pattern_match:
            ...
            if not filtered_matches:
                logger.error(f"[PATTERN-REJECT] All matches for pattern {pattern_idx} were headers, trying next pattern")
                continue
            ...
            logger.error(f"[DISTANCE-DEBUG] Pattern {pattern_idx}: best_distance={best_distance}, context_length={context_length}")
            ...
            if best_distance > context_length:
                logger.error(f"[DISTANCE-DEBUG] REJECTED: Match outside context bounds ({best_distance} > {context_length})")
                continue
            else:
            ...
            try:
                ...
                logger.error(f"[REPORTER-DEBUG] Pattern {pattern_idx}: target_fam='{target_fam}', between_fam='{between_fam}', between_seg='{between_seg[:50]}...'")
                ...
                if between_fam and between_fam != 'unknown' and target_fam and between_fam != target_fam:
                    ...
                    if re.match(parallel_pattern, between_clean):
                        ...
                        if distance_to_next_citation < 20:
                            logger.error(f"[REPORTER-DEBUG] REJECTED: Different citation immediately follows match (not parallel cluster)")
                            continue
                        else:
            ...
            if pattern_idx in [1, 2, 3]:  # Patterns with 2 groups (plaintiff v. defendant)
                ...
                if plaintiff and plaintiff[0].islower():
                    ...
                    if in_match:
                        ...
                        logger.info(f"[STRICT-EXTRACT] Extracted actual plaintiff from 'in' pattern: '{actual_plaintiff}'")
                    ...
                    if not actual_plaintiff:
                        ...
                        if cap_words:
                            ...
                            logger.info(f"[STRICT-EXTRACT] Extracted actual plaintiff from capitalized words: '{actual_plaintiff}'")
                    ...
                    if not actual_plaintiff:
                        ...
                        if v_match:
                            ...
                            logger.info(f"[STRICT-EXTRACT] Extracted actual plaintiff from 'v.' pattern: '{actual_plaintiff}'")
                ...
                if has_et_al and has_role_word:
                    ...
                    role_word_count = sum(1 for role in ['PETITIONER', 'RESPONDENT', 'APPELLANT', 'APPELLEE', 'PLAINTIFF', 'DEFENDANT'] if role in case_name_upper)
                ...
                if has_role_word and has_no:
                    logger.warning(f"[STRICT-EXTRACT] REJECTED header (role word + NO): '{case_name}' (has_role_word={has_role_word}, has_no={has_no})")
                    return None
                
                # THEN: Check detailed patterns
                for pattern in header_patterns:
                    if re.search(pattern, case_name, re.IGNORECASE):
                        logger.warning(f"[STRICT-EXTRACT] REJECTED document header pattern: '{case_name}' (matched pattern: {pattern})")
                        return None  # CRITICAL FIX: Return None immediately, don't try next pattern
                logger.error(f"[EXTRACTION-DEBUG] Built case name: '{case_name}' from plaintiff='{plaintiff}', defendant='{defendant}'")
            ...
            # If it has a role word and NO, it's almost certainly a header
            if has_role_word and has_no:
                logger.error(f"[STRICT-EXTRACT] REJECTED header (role word + NO): '{case_name}' (has_role_word={has_role_word}, has_no={has_no})")
                return None
            ...
            role_word_count = sum(1 for role in ['PETITIONER', 'RESPONDENT', 'APPELLANT', 'APPELLEE', 'PLAINTIFF', 'DEFENDANT'] if role in case_name_upper)
            if role_word_count >= 2:
                logger.error(f"[STRICT-EXTRACT] REJECTED header (multiple role words: {role_word_count}): '{case_name}'")
                return None
            ...
            if ' v. ' in case_name.lower():
                ...
                if plaintiff_part.endswith(('.', ',')) or defendant_part.endswith(('.', ',')):
                    ...
                    logger.error(f"[VALIDATION-DEBUG] Checking trailing punctuation: '{plaintiff_part}' + '{defendant_part}' = '{combined}'")
                    
                    # Check if this is a valid case name with reasonable structure
                    has_valid_plaintiff = len(plaintiff_part.rstrip('.,')) >= 2 and re.search(r'[A-Za-z]{2,}', plaintiff_part)
                    has_valid_defendant = len(defendant_part.rstrip('.,')) >= 2 and re.search(r'[A-Za-z]{2,}', defendant_part)
                    
                    # Allow trailing commas if both parties are valid and not just single letters
                    if (plaintiff_part.endswith(',') or defendant_part.endswith(',')) and has_valid_plaintiff and has_valid_defendant:
                        ...
                        logger.error(f"[VALIDATION-DEBUG] REJECTED: Trailing punctuation without known abbreviation or valid structure")
            ...
            if len(case_name) >= 5:
                ...
                has_et_al_final = (
                    'ET AL' in case_name_upper_final or 
                    'ETAL' in case_name_upper_final.replace(' ', '').replace('.', '').replace(',', '') or
                    re.search(r'ET\s+AL\.?', case_name_upper_final)
                )
                
                # Check for role words (including plurals)
                role_patterns = [
                    r'PETITIONER', r'RESPONDENT', r'APPELLANT', r'APPELLEE', 
                ...
                has_no_final = (
                    'NO.' in case_name_upper_final or 
                    ' NO ' in case_name_upper_final or 
                    case_name_upper_final.endswith(' NO') or
                    re.search(r'\bNO\.?\s*\d+', case_name_upper_final)  # "NO 12345" or "NO. 12345"
                )
                ...
                role_no_pattern = re.search(
                    r'(?:PETITIONER|RESPONDENT|APPELLANT|APPELLEE|PLAINTIFF|DEFENDANT).*NO\.?',
                    case_name_upper_final
                )
                
                # Reject if it's clearly a header
                is_header = (
                    (has_et_al_final and has_role_word_final) or 
                    (has_role_word_final and has_no_final) or
                    header_pattern_match is not None or
                    role_no_pattern is not None
                )
                
                if is_header:
                    logger.error(f"[STRICT-EXTRACT-FINAL-REJECT] REJECTED header pattern in final check: '{case_name}'")
                    logger.error(f"[STRICT-EXTRACT-FINAL-REJECT]   has_et_al={has_et_al_final}, has_role={has_role_word_final}, has_no={has_no_final}")
                    logger.error(f"[STRICT-EXTRACT-FINAL-REJECT]   header_pattern_match={header_pattern_match is not None}, role_no_pattern={role_no_pattern is not None}")
                    return None  # CRITICAL FIX: Return None immediately, don't try next pattern
                ...
                logger.error(f"[FINAL-DEBUG] REJECTED case name too short: '{case_name}' ({len(case_name)} chars) for {citation_text}")
