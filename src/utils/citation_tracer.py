...
class CitationTracer:
    ...
    def end_step(self, citations_count: int = 0, additional_data: Dict[str, Any] = None) -> None:
        ...
        logger.info(
            f"[TRACE-{self.trace_id}] Completed {self.active_step} in {duration_ms:.2f}ms - {citations_count} citations"
        )
    ...
    def log_pipeline_summary(self) -> None:
        ...
        if summary["bottlenecks"]:
            ...
            for bottleneck in summary["bottlenecks"]:
                logger.info(
                    f"    {bottleneck['step']}: {bottleneck['duration_ms']:.2f}ms ({bottleneck['percentage']:.1f}%)"
                )
...
def trace_function(step_name: Optional[str] = None):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            trace_data = {"function": func.__name__, "args_count": len(args), "kwargs": list(kwargs.keys())}
