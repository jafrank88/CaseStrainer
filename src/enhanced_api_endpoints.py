...
from enhanced_async_manager import EnhancedAsyncManager, JobStatus
...
async def initialize_services():
    ...
    input_processor = EnhancedInputProcessor(max_concurrent_jobs=10, timeout_seconds=300)
...
async def handle_single_processing(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if input_type == "text":
            text_result = await input_processor.process_text_input(input_data.get("text", ""), request_id)
        elif input_type == "file":
            text_result = await input_processor.process_file_input(input_data.get("file_path", ""), request_id)
        elif input_type == "url":
            text_result = await input_processor.process_url_input(input_data.get("url", ""), request_id)
        else:
        ...
        # Process citations
        citation_result = await process_citations_from_text(text_result["text"], options, request_id)

        return create_success_response(
            {"request_id": request_id, "input_processing": text_result, "citation_processing": citation_result}
        )
            ...
            if input_result["success"]:
                try:
                    citation_result = await process_citations_from_text(
                        input_result["text"], options, input_result["request_id"]
                    )
                    ...
                    citation_results.append(
                        {
                            ...
                            "input_processing": input_result,
                            "citation_processing": {"success": False, "error": str(e)},
                        }
                    )
            else:
                citation_results.append(
                    {
                        ...
                        "request_id": input_result.get("request_id", f"input_{i}"),
                        "input_processing": input_result,
                        "citation_processing": {"success": False, "error": "Input processing failed"},
                    }
                )
...
async def process_citations_from_text(text: str, options: Dict[str, Any], request_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Process citations
        result = citation_processor.process({"type": "text", "text": text}, request_id)
        ...
        return {"success": False, "error": str(e), "citations": [], "clusters": [], "processing_stats": {}}
        ...
        async def process_job_func(job_input_data, progress_callback=None):
            ...
            if input_type == "text":
                text_result = await input_processor.process_text_input(
                    job_input_data.get("text", ""), job_input_data.get("request_id")
                )
            elif input_type == "file":
                text_result = await input_processor.process_file_input(
                    job_input_data.get("file_path", ""), job_input_data.get("request_id")
                )
            elif input_type == "url":
                text_result = await input_processor.process_url_input(
                    job_input_data.get("url", ""), job_input_data.get("request_id")
                )
            else:
            ...
            # Process citations
            citation_result = await process_citations_from_text(
                text_result["text"], options, job_input_data.get("request_id")
            )

            if progress_callback:
                await progress_callback(100, "Completed", "Processing completed")

            return {"input_processing": text_result, "citation_processing": citation_result}

        # Submit job
        submitted_job_id = await async_manager.submit_job(
            input_data=input_data, processor_func=process_job_func, job_id=job_id
        )

        return create_success_response(
            {"job_id": submitted_job_id, "status": "submitted", "message": "Job submitted for processing"}
        )
...
async def get_job_status(job_id: str):
    ...
    try:
        ...
        return create_success_response({"job": job_status})
...
async def get_job_result(job_id: str):
    ...
    try:
        ...
        return create_success_response({"job_id": job_id, "result": job_result})
...
async def list_jobs():
    ...
    try:
        ...
        return create_success_response({"jobs": jobs, "total_count": len(jobs)})
...
async def get_stats():
    ...
    try:
        ...
        return create_success_response(
            {"async_manager": manager_stats, "input_processor": input_stats, "system": system_stats}
        )
...
def get_documentation():
    ...
    docs = {
        ...
        "endpoints": {
            ...
            "/jobs/<job_id>": {"method": "GET", "description": "Get job status and progress information"},
            "/jobs/<job_id>/result": {"method": "GET", "description": "Get job result if completed"},
            "/jobs/<job_id>/cancel": {"method": "POST", "description": "Cancel a running job"},
            "/stats": {"method": "GET", "description": "Get system statistics and performance metrics"},
            "/health": {"method": "GET", "description": "Enhanced health check with service status"},
            "/documentation": {"method": "GET", "description": "Get this API documentation"},
        },
        "input_types": {
            "text": {
                "description": "Direct text input",
                "fields": {"type": "text", "text": "string (required) - Text containing citations"},
            },
            "file": {
            ...
            "url": {
                ...
                "fields": {"type": "url", "url": "string (required) - URL to process"},
            },
            "batch": {
