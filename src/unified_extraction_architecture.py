...
class UnifiedExtractionArchitecture:
    ...
    def _setup_patterns(self):
        ...
        self.contamination_patterns = [
            ...
            r"\b(compare|contrast|but\s+see|see\s+also)\b",
    ...
    def extract_case_name_and_year(
        ...
        if debug:
            logger.warning(
                f"[DEBUG] UNIFIED_EXTRACT: Starting extraction for citation '{citation}' at position {start_index}-{end_index}"
            )
        ...
        if not result or not result.case_name or result.case_name == "N/A":
            result = self._extract_with_patterns(
                normalized_text, citation, start_index or 0, end_index or len(normalized_text), debug
            )
        ...
        if debug:
            logger.warning(
                f"[SUCCESS] UNIFIED_EXTRACT: Final result for '{citation}': '{result.case_name}' (confidence: {result.confidence:.2f})"
            )

        return result

    def _try_patterns_on_context(
        self, context: str, citation: str, citation_start_in_context: int, citation_end_in_context: int, debug: bool
    ...
    def _clean_case_name_advanced(self, case_name: str, debug: bool = False) -> str:
        ...
        if not re.match(r"^In\s+re\s+", case_name, re.IGNORECASE):
            ...
            if not any(
                term in case_name for term in [", Inc.", ", Inc", " Corp.", " Corp", " LLC", " L.L.C.", " Ltd.", " Ltd"]
            ):
    ...
    def _extract_with_context(
        ...
        try:
            ...
            if quoting_match:
                ...
                if debug:
                    logger.warning(
                        f"[DEBUG] QUOTING_DETECTED: Found 'quoting {quoted_case_name}' at distance {distance_to_citation}"
                    )
                ...
                if distance_to_citation < 200 and distance_to_citation > 0:
                    ...
                    if citation in text_after_quoting:
                        if debug:
                            ...
                            logger.warning(
                                f"[SUCCESS] NESTED_CITATION_SUCCESS: Citation '{citation}' found after 'quoting', using quoted case name '{quoted_case_name}'"
                            )
                        ...
                        if debug:
                            logger.warning(
                                f"[DEBUG] QUOTING_SKIP: Citation '{citation}' appears before 'quoting', not part of quoted case"
                            )
                else:
                    if debug:
                        logger.warning(
                            f"[DEBUG] QUOTING_SKIP: Distance {distance_to_citation} outside valid range for quoting pattern"
                        )
            ...
            if debug:
                ...
                logger.warning(
                    f"[DEBUG] VACATUR_DEBUG: Search window ({len(search_back)} chars): '{search_back[-200:]}'"
                )
            ...
            for vacatur_pattern in vacatur_patterns:
                ...
                if debug:
                    logger.warning(
                        f"[DEBUG] VACATUR_DEBUG: Pattern '{vacatur_pattern}' -> {'FOUND' if vacatur_match else 'NOT FOUND'}"
                    )
                ...
                if vacatur_match:
                    ...
                    if debug:
                        logger.warning(
                            f"[DEBUG] VACATUR_DEBUG: Found {len(case_matches)} case name matches before vacatur"
                        )
                        if case_matches:
                            for i, match in enumerate(case_matches):
                                logger.warning(f"[DEBUG] VACATUR_DEBUG: Match {i+1}: '{match.group(0)}'")
                        logger.warning(
                            f"[DEBUG] VACATUR_DEBUG: Text before vacatur ({len(text_before_vacatur)} chars): '{text_before_vacatur[-200:]}'"
                        )
                    ...
                    if case_matches:
                        ...
                        if self._is_valid_case_name(case_name):
                            year = self._extract_year_from_context(
                                text[max(0, start_index - 300) : start_index + 50], citation
                            )
                            return ExtractionResult(
                                case_name=case_name,
                                year=year,
                    ...
                    if debug:
                        logger.warning(
                            f"[DEBUG] VACATUR_SKIP: Found '{vacatur_pattern}' but couldn't extract case name before it"
                        )
            ...
            if debug:
                ...
                try:
                    ...
                    logger.warning(
                        "🧠 CONTEXT_FLOW: Wrote fallback context snapshots to 'fallback_extended_context.txt' and 'fallback_extended_context_full.txt'"
                    )
                except Exception as e:
                ...
                if matches:
                    ...
                    for match in matches:
                        ...
                        if i < 2 and len(match.groups()) >= 3:  # Patterns 1 and 2 have volume as group 3
                            ...
                            if citation_volume and match_volume == citation_volume:
                                ...
                                if debug:
                                    logger.warning(
                                        f"ðŸ” UNIFIED_EXTRACT: Volume match! Citation: {citation_volume}, Match: {match_volume}"
                                    )
                        ...
                        if distance < 100:  # Within 100 characters
                            ...
                            has_contamination = any(
                                contam.lower() in match_text.lower() for contam in contamination_indicators
                            )
                            ...
                            if not has_contamination:
                                if i < 2 and volume_match:
                                    ...
                                    if debug:
                                        logger.warning(
                                            f"ðŸ” UNIFIED_EXTRACT: Volume-matched case name: '{match_text}' at distance {distance}"
                                        )
                                    ...
                                    if case_name_quality > 0.5:  # Only consider high-quality case names
                                        if distance < min_distance:
                                            ...
                                            if debug:
                                                logger.warning(
                                                    f"[DEBUG] UNIFIED_EXTRACT: New best match: '{match_text}' at distance {distance} (quality: {case_name_quality})"
                                                )
                            elif debug:
                                logger.warning(
                                    f"ðŸ” UNIFIED_EXTRACT: Match rejected due to contamination: '{match_text}'"
                                )
                    ...
                    if best_match:
                        ...
                        if len(groups) == 1:
                            ...
                            logger.warning(
                                f"[DEBUG] DEBUG_TRUNCATION: Raw plaintiff: '{raw_plaintiff}', Raw defendant: '{raw_defendant}'"
                            )
                            ...
                            logger.warning(
                                f"[DEBUG] DEBUG_TRUNCATION: Cleaned plaintiff: '{plaintiff}', Cleaned defendant: '{defendant}'"
                            )
                        ...
                        if self._is_valid_case_name(case_name):
                            ...
                            if debug:
                                logger.warning(
                                    f"🧠 CONTEXT_FLOW: Returning case from context_pattern_{i+1}: '{case_name}'"
                                )
                            # CRITICAL: Do NOT set canonical_name during extraction
                            return ExtractionResult(
                                case_name=case_name,
            ...
            for i, pattern in enumerate(pre_citation_patterns):
                ...
                if matches:
                    ...
                    if self._is_valid_case_name(case_name):
                        ...
                        if debug:
                            logger.warning(
                                f"🧠 CONTEXT_FLOW: Returning case from pre_citation_pattern_{i+1}: '{case_name}'"
                            )
                        # CRITICAL: Do NOT set canonical_name during extraction
                        return ExtractionResult(
                            case_name=case_name,
            ...
            for i, pattern in enumerate(targeted_patterns):
                ...
                if matches:
                    ...
                    for match in matches:
                        ...
                        distance = min(
                            abs(match_start - citation_pos_in_targeted), abs(match_end - citation_pos_in_targeted)
                        )
                ...
                if matches:
                    ...
                    if best_match:
                        ...
                        if len(groups) == 1:
                            ...
                            continue
                        logger.warning(
                            f"ðŸ” DEBUG_TRUNCATION_FALLBACK: Raw plaintiff: '{raw_plaintiff}', Raw defendant: '{raw_defendant}'"
                        )
                        ...
                        defendant = clean_extracted_case_name(raw_defendant)
                        logger.warning(
                            f"ðŸ” DEBUG_TRUNCATION_FALLBACK: Cleaned plaintiff: '{plaintiff}', Cleaned defendant: '{defendant}'"
                        )
    ...
    def _clean_case_name(self, name: str) -> str:
        ...
        signal_words = [
            ...
            r"^Under the\s+",
            r"^In the case of\s+",
            r"^The Supreme Court in\s+",
            r"^The court in\s+",
            r"^In\s+",
            r"^As discussed in\s+",
            r"^As noted in\s+",
            r"^As found in\s+",
            r"^As determined in\s+",
            r"^As ruled in\s+",
            r"^Following\s+",
            r"^Based on\s+",
    ...
    def _is_valid_case_name(self, case_name: str) -> bool:
        ...
        contamination_indicators = [
            ...
            "f.supp.2d",
            "wl ",
            "u.s. ",  # Space after to avoid matching "us" in words
        ]
        ...
        if " v. " in case_name:
            ...
            if len(parts) == 2:
                ...
                if (
                    ...
                    and len(plaintiff) >= 2
                    and len(defendant) >= 2
                    and not plaintiff[0].islower()
                    and not defendant[0].islower()
                ):
                    return True

        # 2. "In re" or "In the Matter of" pattern
        in_re_match = re.match(r"^(In\s+re\s+|In\s+the\s+Matter\s+of\s+|Matter\s+of\s+)(.+)", case_name, re.IGNORECASE)
        if in_re_match:
    ...
    def _extract_case_name_intelligent(
        self, text: str, citation: str, start_index: int, end_index: int, debug: bool
    ) -> Optional[ExtractionResult]:
        """
        ...
        if doc_length <= 2000:  # Very small document
            ...
            extended_after = (
                min(1500, doc_length - end_index) if doc_length > 50000 else min(1000, doc_length - end_index)
            )
        ...
        for v_match in v_patterns:
            ...
            if debug and (not plaintiff or not defendant):
                logger.warning(
                    f"🧠 INTELLIGENT_EXTRACT: Skipping candidate around position {v_start}-{v_end} (plaintiff='{plaintiff}', defendant='{defendant}')"
                )
            ...
            if plaintiff and defendant:
                ...
                if debug:
                    logger.warning(
                        f"🧠 INTELLIGENT_EXTRACT: Found potential case '{case_name}' (confidence={confidence:.2f}, distance={distance})"
                    )

                # Choose the best case name based on confidence and distance
                if confidence > best_confidence or (confidence == best_confidence and distance < best_distance):
                    if self._is_valid_case_name(case_name):
        ...
        if debug:
            logger.warning(
                f"🧠 INTELLIGENT_EXTRACT: Post-processing best_case='{best_case_name}', cleaned='{cleaned_best_case}', confidence={best_confidence:.2f}"
            )
            logger.warning(
                f"🧠 INTELLIGENT_EXTRACT: Preparing to invoke fallback with extended_context_len={len(extended_context)}"
            )
        ...
        if debug:
            ...
            logger.warning(
                f"🧠 INTELLIGENT_EXTRACT: Fallback helper returned {fallback_str} using preview='{preview}...'"
            )
        ...
        if fallback_case:
            ...
            if cleaned_fallback:
                if debug:
                    logger.warning(
                        f"🧠 FALLBACK_CITATION: Cleaned fallback='{cleaned_fallback}', best='{cleaned_best_case}', best_confidence={best_confidence:.2f}"
                    )
            ...
            if use_fallback:
                if debug:
                    logger.warning(
                        f"🧠 FALLBACK: Using direct pattern recovery '{cleaned_fallback}' (overriding best='{cleaned_best_case}')"
                    )
                # CRITICAL: Do NOT set canonical_name during extraction
                return ExtractionResult(
        ...
        if cleaned_best_case and cleaned_best_case != "N/A" and best_confidence > 0.6:  # Minimum confidence threshold
            ...
            if debug:
                logger.warning(
                    f"🧠 INTELLIGENT_EXTRACT: Returning best candidate '{cleaned_best_case}' (confidence={best_confidence:.2f})"
                )
            # CRITICAL: Do NOT set canonical_name during extraction
            return ExtractionResult(
        ...
        if fallback_case:
            ...
            if cleaned_fallback:
                ...
                from .utils.strict_context_isolator import (
                    _expand_abbreviations,
                    _add_missing_words,
                    _fix_formatting_issues,
                )
                ...
                if cleaned_fallback != original_case_name:
                    logger.info(
                        f"[ACCURACY] Abbreviation expansion (fallback): '{original_case_name}' → '{cleaned_fallback}'"
                    )
                ...
                if cleaned_fallback != original_case_name:
                    logger.info(
                        f"[ACCURACY] Missing words added (fallback): '{original_case_name}' → '{cleaned_fallback}'"
                    )
                ...
                if cleaned_fallback != original_case_name:
                    logger.info(
                        f"[ACCURACY] Formatting fixed (fallback): '{original_case_name}' → '{cleaned_fallback}'"
                    )

                if debug:
                    logger.warning(
                        f"🧠 FALLBACK: Using direct pattern recovery '{cleaned_fallback}' (best case invalid)"
                    )
                # CRITICAL: Do NOT set canonical_name during extraction
                return ExtractionResult(
                    case_name=cleaned_fallback,
        ...
        if debug:
            logger.warning(
                f"🧠 No valid case name found using enhanced extraction (cleaned_best_case={cleaned_best_case}, confidence={best_confidence:.2f}, fallback_case={fallback_case})"
            )
            logger.warning("🧠 INTELLIGENT_EXTRACT: Returning None after exhausting intelligent and fallback paths")
        return None

    def _recover_case_name_from_citation_pattern(
        self, context: str, citation: str, debug: bool
        ...
        def log_debug(message: str, level: str = "info") -> None:
            ...
            log_method = {"warning": logger.warning, "info": logger.info, "error": logger.error}.get(
        ...
        def find_corporate_name(text: str, end_pos: int) -> Optional[Tuple[str, int, int]]:
            ...
            patterns = [
                ...
                r'([A-Z][\w\s,&\-"\']*?(?:\s*(?:'
                + "|".join(re.escape(s) + r"" for s in CORPORATE_SUFFIXES)
                + r'))\s*,\s*[A-Z][\w\s,&\-"\']*?)\s+v\b',
                # Corporate names without comma before 'v.'
                r'([A-Z][\w\s,&\-"\']*?(?:\s*(?:'
                + "|".join(re.escape(s) + r"\b" for s in CORPORATE_SUFFIXES)
                + r"))\s*[^,;()]*?)\s+v\b",
        ...
        def normalize_candidate(raw: str) -> Optional[str]:
            ...
            candidate = re.sub(
                r"(?:,\s*)?(?:\d{1,4}\s+[A-Za-z\.]+\s+\d{1,4}(?:\s*[A-Za-z\.]+\s*\d{1,4})*)(?:.*)$", "", candidate
            ).strip(" ,;:.")
        ...
        def register_candidate(text: str, start_idx: int, end_idx: int, source: str) -> None:
            ...
            candidate = Candidate(text=text, start=start_idx, end=end_idx, source=source)
            ...
            candidate.preceded_by_signal = any(
                re.search(pattern, preceding_text, re.IGNORECASE) for pattern in signal_patterns
            )
        ...
        def find_candidates() -> List[Candidate]:
                    ...
                    try:
                        ...
                        for match in re.finditer(pattern, search_text, re.IGNORECASE | re.DOTALL):
                            ...
                            if len(candidate_text) >= 5:  # Minimum reasonable length
                                register_candidate(candidate_text, abs_start, abs_end, f"{source}_{i}")
            ...
            if citation_index > 0:
                ...
                case_name_pattern = (
                    r"\b([A-Z][A-Za-z0-9\'’&\.,\-\s]{2,50}?\s+v(?:s|\.?)\s+[A-Z][A-Za-z0-9\'’&\.,\-\s]{2,})"
                )
                ...
                for match in re.finditer(case_name_pattern, pre_segment, re.IGNORECASE):
                    ...
                    register_candidate(match.group(1).strip(), abs_start, abs_end, "nearest_v_pattern")
        ...
        if citation_index > 0:
            ...
            if matches:
                ...
                valid_matches = [
                    m for m in matches if not re.search(r"\bv(?:s|\\.?)\\s+\d+\\s+[A-Za-z\\.]+\\s+\\d+", m.group(0))
                ]
        ...
        def score_candidate(candidate: Candidate) -> float:
            ...
            corporate_pattern = r"\b(?:" + "|".join(re.escape(suffix) + r"\b" for suffix in CORPORATE_SUFFIXES) + r")"
            ...
            if has_corporate:
                ...
                if re.search(r",\s*(?:" + "|".join(re.escape(s) + r"\b" for s in CORPORATE_SUFFIXES) + r")", text):
                    score += 1.0

            # Washington State patterns
            is_washington_state = bool(re.search(r"\bState\s+(?:of\s+)?(?:ex\s+rel\.?\s+)?v\.?\s+[A-Z]", text))
            is_washington_admin = bool(
                re.search(r"\b(?:In\s+re|Matter\s+of|In\s+the\s+Matter\s+of)\s+[A-Z]", text, re.IGNORECASE)
            )
        ...
        def select_best_candidate(candidates: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
            ...
            if any(
                any(term in c["text"] for term in [" v. ", " Inc", " Corp", " LLC", " Ltd", " LP"])
        ...
        try:
            ...
            if not best_candidate:
                ...
                if year_match:
                    ...
                    return {"extracted_case_name": None, "extracted_date": int(year_match.group(0))}
                return None
            ...
            return {"extracted_case_name": case_name, "extracted_date": year}
    ...
    def _prefer_canonical_year(
        self, extracted_year: Optional[str], canonical_metadata: Dict[str, Any]
    ) -> Optional[str]:
        return prefer_canonical_year(extracted_year, canonical_metadata)

    def _ensure_case_name(
        self, result: ExtractionResult, citation: str, context: str = None, debug: bool = False
    ) -> ExtractionResult:
        """
        ...
        if context:
            ...
            if recovery and recovery.get("extracted_case_name"):
                ...
                if debug:
                    logger.warning(
                        f"[SUCCESS] CASE_NAME_RECOVERED: Recovered case name '{result.case_name}' from context"
                    )
                return result
        ...
        if not result or not hasattr(result, "case_name") or result.case_name == "N/A":
            ...
            if simple_name:
                ...
                if debug:
                    logger.warning(
                        f"[WARNING] CASE_NAME_EXTRACTED: Extracted basic case name '{result.case_name}' from citation"
                    )
            else:
...
def extract_case_name_and_year_unified(
    text: str, citation: str, start_index: Optional[int] = None, end_index: Optional[int] = None, debug: bool = False
) -> Dict[str, Any]:
    """
    ...
    return extract_case_name_and_date_unified_master(
        text=text, citation=citation, start_index=start_index, end_index=end_index, debug=debug
    )
