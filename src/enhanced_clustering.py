...
class EnhancedCitationClusterer:
    ...
    def _compile_parallel_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "parallel_indicators": re.compile(
                r"\b(?:see\s+also|also|accord|cf\.?|compare|citing|quoting)\b", re.IGNORECASE
            ),
    ...
    def cluster_citations(
        self, citations: List[Any], text: str = "", request_id: str = "", enable_verification: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        return cluster_citations_unified_master(
            citations=citations, original_text=text, enable_verification=enable_verification, request_id=request_id
        )
    ...
    def _group_by_case_name_and_year(self, citations: List[Any]) -> List[List[Any]]:
        ...
        for citation in citations:
            ...
            if isinstance(citation, dict):
                ...
                case_name = getattr(citation, "original_case_name", getattr(citation, "extracted_case_name", None))
                year = getattr(citation, "original_date", getattr(citation, "extracted_date", None))
        ...
        if self.config.debug_mode and result:
            for i, group in enumerate(result):
                first_cite = group[0]
                case_name = getattr(
                    first_cite, "original_case_name", getattr(first_cite, "extracted_case_name", "Unknown")
                )
                year = getattr(first_cite, "original_date", getattr(first_cite, "extracted_date", "Unknown"))
    ...
    def _extract_and_propagate_metadata_enhanced(
        self, citations: List[Any], groups: List[List[Any]], text: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        for group in groups:
            ...
            if isinstance(first_citation, dict):
                first_case_name = first_citation.get("original_case_name", first_citation.get("extracted_case_name"))
            else:
                first_case_name = getattr(
                    first_citation, "original_case_name", getattr(first_citation, "extracted_case_name", None)
                )
            ...
            if isinstance(last_citation, dict):
                last_year = last_citation.get("original_date", last_citation.get("extracted_date"))
            else:
                last_year = getattr(last_citation, "original_date", getattr(last_citation, "extracted_date", None))
            ...
            for citation in group:
                ...
                if isinstance(citation, dict):
                    original_case_name = citation.get("original_case_name", citation.get("extracted_case_name"))
                    original_date = citation.get("original_date", citation.get("extracted_date"))
                    ...
                    original_case_name = getattr(
                        citation, "original_case_name", getattr(citation, "extracted_case_name", None)
                    )
                    original_date = getattr(citation, "original_date", getattr(citation, "extracted_date", None))
                ...
                if isinstance(citation, dict):
                    enhanced_citation["metadata"].update(
                        {k: v for k, v in citation.items() if k not in enhanced_citation and not k.startswith("_")}
                    )
                else:
                    enhanced_citation["metadata"].update(
                        {
    ...
    def _create_final_clusters_enhanced(self, enhanced_citations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for (case_name, year), citations in clusters.items():
            if not case_name or case_name == "N/A":
                ...
                for citation in citations:
                    alt_name = citation.get("verified_case_name") or citation.get("extracted_case_name")
    ...
    def clear_cache(self):
        ...
        expired_keys = [
            key for key, timestamp in self._cache_timestamps.items() if current_time - timestamp > self.config.cache_ttl
        ]
