...
class ProcessingConfig:
    ...
    external_apis: List[str] = field(default_factory=lambda: ["justia", "openjurist", "cornell_lii", "google_scholar"])
...
class SimplifiedCitationProcessor:
    ...
    def _process_async(self, text: str, request_id: str) -> ProcessingResult:
        ...
        return ProcessingResult(
            ...
            metadata={"request_id": request_id, "status": "queued", "job_id": job.id},
        )
    ...
    def _verify_citations(self, citations: List[Dict[str, Any]], request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    results = executor.submit(run_optimized_batch_verification).result(
        ...
        logger.info(
            f"[{request_id}] Optimized verification complete: {verified_count} verified, {possible_matches} possible matches"
        )
