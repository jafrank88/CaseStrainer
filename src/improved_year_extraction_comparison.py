...
def extract_citations_with_years_from_body(text: str) -> List[Dict[str, Any]]:
    ...
    def extract_procedural_citations(
        text: str, base_citation: str, base_year: str, start_pos: int, end_pos: int
    ) -> List[Dict[str, Any]]:
    ...
    for pattern in citation_patterns:
        ...
        for match in matches:
            ...
            if year and validate_year(year):
                ...
                procedural_citations = extract_procedural_citations(
                    text, citation_text, year, citation_start, citation_end
                )
    ...
    try:
        ...
        for entry in toa_entries:
            for citation in entry.citations:
                ...
                citations.append({"citation": citation, "year": year, "case_name": entry.case_name, "source": "ToA"})
    ...
    for citation_key in all_citations:
        ...
        if toa_cit and body_cit:
            ...
            if toa_name == body_name and toa_year == body_year:
                results["matching_citations"].append({"citation": citation_key, "toa": toa_cit, "body": body_cit})
            else:
