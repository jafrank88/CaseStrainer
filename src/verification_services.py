...
class CourtListenerService:
    ...
    def __init__(self):
        ...
        headers = {"User-Agent": "CaseStrainer/1.0 (Legal Citation Verification Tool)", "Accept": "application/json"}
    ...
    def verify_citations_batch(self, citations: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            batch_data = {"text": " ".join(citations)}

            logger.info(f"Verifying {len(citations)} citations with CourtListener citation lookup")

            response = self.session.post(self.citation_lookup_url, json=batch_data, timeout=DEFAULT_REQUEST_TIMEOUT)
    ...
    def verify_citations_search(self, citations: List[str]) -> Dict[str, Any]:
        ...
        for citation in citations:
            ...
            try:
                ...
                response = self.session.get(self.search_url, params=search_params, timeout=DEFAULT_REQUEST_TIMEOUT)
