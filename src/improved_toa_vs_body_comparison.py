...
def safe_extract_toa_section(text: str) -> str:
    ...
    if len(toa_text) > config.MAX_TOA_SIZE:
        logger.info(
            f"[SAFE TOA] ToA section too large ({len(toa_text):,} chars), truncating to {config.MAX_TOA_SIZE:,}"
        )
...
def compare_citations(toa_citations: List[Dict], body_citations: List[Dict]) -> Dict[str, Any]:
    ...
    results = {
        ...
        "different_names": [],
        "different_years": [],
        "stats": {"toa_count": len(toa_citations), "body_count": len(body_citations)},
    }
    ...
    for citation_key in all_citations:
        ...
        if toa_cit and body_cit:
            ...
            if toa_name == body_name and toa_year == body_year:
                results["matching"].append({"citation": citation_key, "toa": toa_cit, "body": body_cit})
            else:
