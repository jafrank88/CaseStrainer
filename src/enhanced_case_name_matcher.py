...
class EnhancedCaseNameMatcher:
    ...
    def calculate_similarity(self, name1: str, name2: str) -> float:
        ...
        combined_similarity = (
            0.25 * seq_similarity + 0.30 * word_similarity + 0.20 * substring_similarity + 0.25 * key_word_similarity
        )
    ...
    def _extract_key_words(self, normalized_name: str) -> set:
        ...
        key_words = set()
        for word in words:
            if len(word) > 2 and word not in legal_stop_words and not word.isdigit():
                key_words.add(word)
    ...
    def find_best_match(
        self, target_name: str, candidate_names: List[str], threshold: float = 0.6
    ...
    def normalize_for_verification(self, case_name: str) -> str:
        ...
        for suffix in ["petitioner", "respondent", "appellant", "appellee", "defendant", "plaintiff", "relator"]:
            if normalized.endswith(" " + suffix):
...
def find_best_match(
    target_name: str, candidate_names: List[str], threshold: float = 0.6
) -> Optional[Tuple[str, float]]:
    """Convenience function for backward compatibility."""
