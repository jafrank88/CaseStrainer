...
class UltraFastPDFProcessor:
    ...
    def __init__(self):
        super().__init__()
        self._critical_fixes = re.compile(r"(\d+)\s*([USF])\.\s*([SCtEd]+)\.\s*[\n\-]\s*(\d+)")
...
class UnifiedDocumentProcessor:
    ...
    def clean_extracted_text(self, text: str) -> str:
        ...
        try:
            text = "".join(char for char in text if char.isprintable() or char in ".,;:()[]{}")
            text = re.sub(r"(\d+)\s*[Uu]\.\s*[Ss]\.\s*(\d+)", r"\1 U.S. \2", text)
            text = re.sub(r"(\d+)\s*[Ss]\.\s*[Cc][Tt]\.\s*(\d+)", r"\1 S.Ct. \2", text)
            text = re.sub(r"(\d+)\s*[Ll]\.\s*[Ee][Dd]\.\s*(\d+)", r"\1 L.Ed. \2", text)
            text = re.sub(r"(\d+)\s*[Ff]\.\s*(2[Dd]|3[Dd]|4[Tt][Hh])?\s*(\d+)", r"\1 F.\2 \3", text)
    ...
    def convert_pdf_to_markdown(self, pdf_path: str) -> str:
        ...
        try:
            ...
            try:
                proc = subprocess.run(["pdf2md", "-o", temp_path, pdf_path], capture_output=True, text=True)
    ...
    def extract_text_from_url(self, url: str) -> str:
        ...
        try:
            ...
            response = requests.get(
                url, headers=headers, timeout=DEFAULT_REQUEST_TIMEOUT, stream=True, allow_redirects=True
            )
    ...
    def _convert_citation_to_dict(self, citation) -> Dict[str, Any]:
        ...
        if hasattr(citation, "__dict__"):
            ...
            logger.debug(
                f"Data separation maintained - cluster: '{cluster_case_name}', extracted: '{extracted_name}', canonical: '{canonical_name}'"
            )
        ...
        try:
            ...
            return {"citation": str(citation), "error": "Unable to convert citation object"}
    ...
    async def process_document(
        ...
        try:
            ...
            logger.info(
                f"[PROCESS] Preprocessed text (first 500 chars): {preprocessed_text[:500] if preprocessed_text else 'EMPTY'}"
            )
                ...
                try:
                    if UNIFIED_PROCESSOR_AVAILABLE:
                        ...
                        if hasattr(self.citation_processor, "process_text"):
                            ...
                            for result in enhanced_results:
                                citation_dict = {
                                    ...
                                    "extracted_case_name": result.get("shared_case_name")
                                    or result.get("enhanced_case_name")
                                    or result.get("original_case_name"),
                                    "canonical_name": result.get("canonical_name"),
                                    "extracted_date": result.get("shared_year")
                                    or result.get("enhanced_year")
                                    or result.get("original_year"),
                        ...
                        statistics = {
                            ...
                            "unique_cases": len(
                                set(
                                    c.get("extracted_case_name") or c.get("cluster_case_name")
                                    for c in formatted_citations
                                    if (c.get("extracted_case_name") and c.get("extracted_case_name") != "N/A")
                                    or (c.get("cluster_case_name") and c.get("cluster_case_name") != "N/A")
                                )
                            ),
            ...
            result = {
                ...
                "processor_used": (
                    "unified_processor_v2"
                    if UNIFIED_PROCESSOR_AVAILABLE
                    else "enhanced_processor" if ENHANCED_PROCESSOR_AVAILABLE else "none"
                ),
            }
