...
class CitationParser:
            ...
            if last_citation_pos > 0:
                ...
                if (citation_index - context_start) - last_citation_pos < 150:
                    ...
                    if sentence_matches:
                        ...
                        logger.info(
                            f"[DEBUG] No sentence boundary after previous citation, using position after citation"
                        )
                        ...
                        if i > 0 and context_before[i] in " \t\n":
                            if i + 1 < len(context_before):
                                ...
                                if (
                                    ...
                                    and word.lower() not in LEGAL_ABBREVIATIONS
                                    and len(word) > 2
                                ):  # Exclude very short words like "v."
                                    case_start = word_end
                                    logger.info(
                                        f"[DEBUG] Found case name boundary at word: '{word}', case starts at position {case_start}"
                                    )
                                    break
                        ...
                        if v_matches:
                            ...
                            for sentence in reversed(sentences):
                                if "v." in sentence:
                                    v_matches = list(
                                        re.finditer(
                                            r"([A-Z][A-Za-z&\s,\.\'\-]*?\s+v\.\s+[A-Z][A-Za-z&\s,\.\'\-]*?)", sentence
                                        )
                                    )
                                    if v_matches:
                                        ...
                                        if self._is_valid_case_name(potential_case_name):
                                            ...
                                            last_match_end = comma_pos
                                            logger.info(
                                                f"[DEBUG] Found case name by sentence fallback: '{potential_case_name}'"
                                            )
                                            break
    ...
    def _extract_year_from_context(
        self, context_before: str, citation: str, context_after: Optional[str] = None
    ) -> Optional[str]:
...
class DateExtractor:
    ...
    def extract_date_from_context(
        text: str, citation_start: int, citation_end: int, context_window: int = 300
    ) -> Optional[str]:
        """
...
def safe_set_extracted_date(citation, new_date, source="unknown"):
    ...
    try:
        ...
        if (
            not current_date
            or current_date.strip() == ""
            or current_date.strip().upper() in ["N/A", "NONE", "NULL", "UNKNOWN"]
        ):
