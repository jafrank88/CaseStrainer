...
class CitationVerifier(ICitationVerifier):
    ...
    async def _verify_with_courtlistener(self, citation: CitationResult) -> Dict[str, Any]:
        ...
        try:
            ...
            params = {"q": normalized_citation, "type": "o", "format": "json"}  # Opinions

            headers = {"Authorization": f"Token {self.courtlistener_api_key}", "User-Agent": "CaseStrainer/1.0"}
    ...
    def _apply_verification_result(self, citation: CitationResult, result: Dict[str, Any], source: str) -> None:
        ...
        if result.get("canonical_name") and result_source in trusted_api_sources:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"🚫 REJECTED canonical_name from untrusted source '{result_source}': {result.get('canonical_name')}"
            )
        ...
        if result.get("canonical_date") and result_source in trusted_api_sources:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"🚫 REJECTED canonical_date from untrusted source '{result_source}': {result.get('canonical_date')}"
            )

        if result.get("canonical_url") or result.get("url"):
