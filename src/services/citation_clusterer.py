...
class CitationClusterer(ICitationClusterer):
    ...
    def cluster_citations(self, citations: List[CitationResult]) -> List[Dict[str, Any]]:
        ...
        return cluster_citations_unified_master(citations=citations, config=self.config)
    ...
    def _are_citations_parallel(self, citation1: CitationResult, citation2: CitationResult) -> bool:
        ...
        if (
            citation1.canonical_name
            and citation2.canonical_name
            and citation1.canonical_date
            and citation2.canonical_date
        ):
            return (
                citation1.canonical_name.lower() == citation2.canonical_name.lower()
                and citation1.canonical_date == citation2.canonical_date
            )

        if (
            citation1.extracted_case_name
            and citation2.extracted_case_name
            and citation1.extracted_date
            and citation2.extracted_date
        ):
            return (
                self._normalize_case_name(citation1.extracted_case_name)
                == self._normalize_case_name(citation2.extracted_case_name)
                and citation1.extracted_date == citation2.extracted_date
            )
        ...
        for citation in citations:
            ...
            if citation.extracted_case_name and citation.extracted_date:
                ...
                cluster_key = create_cluster_key_from_extracted_data(
                    citation.extracted_case_name, citation.extracted_date, citation.citation
                )
    ...
    def _merge_unverified_with_canonical(
        self, citations: List[CitationResult], canonical_clusters: Dict[str, List[CitationResult]]
    ) -> None:
        ...
        for citation in citations:
            ...
            if not citation.verified and citation.extracted_case_name and citation.extracted_date:
                ...
                for cluster_key, cluster_members in canonical_clusters.items():
                    for cluster_member in cluster_members:
                        if cluster_member.canonical_name and cluster_member.canonical_date:

                            normalized_canonical = self._normalize_case_name(cluster_member.canonical_name)

                            if (
                                normalized_extracted == normalized_canonical
                                and citation.extracted_date == cluster_member.canonical_date
                            ):
    ...
    def _create_extracted_clusters(
        self, citations: List[CitationResult], existing_clusters: Dict[str, List[CitationResult]]
    ) -> Dict[str, List[CitationResult]]:
        ...
        for citation in citations:
            ...
            if citation.extracted_case_name and citation.extracted_date:
                ...
                key = (self._normalize_case_name(citation.extracted_case_name), citation.extracted_date)
        ...
        for (case_name, date), members in extracted_groups.items():
            if len(members) > 1:
                ...
                cluster_key = create_cluster_key_from_extracted_data(
                    case_name, date, members[0].citation if members else ""
                )
    ...
    def _citations_refer_to_same_case(self, citation1: str, citation2: str) -> bool:
        ...
        return (
            parts1["volume"] == parts2["volume"]
            and parts1["page"] == parts2["page"]
            and self._reporters_compatible(parts1["reporter"], parts2["reporter"])
        )
