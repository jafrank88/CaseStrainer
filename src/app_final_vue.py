...
def verify_citation_with_extraction(
    citation_text: str, document_text: str = "", api_key: Optional[str] = None
    ...
    try:
        if document_text and document_text.strip():
            ...
            if extraction_result:
                ...
                extracted_name = (
                    extraction_result.get("case_name") or extraction_result.get("extracted_case_name") or "N/A"
                )

                extracted_date = extraction_result.get("year") or extraction_result.get("date") or "N/A"
                ...
                logger.info(
                    f"Extraction complete - extracted: {result['extracted_case_name']}, canonical: {result['canonical_name']}"
                )
            else:
    ...
    logger.info(
        f"Final result for {citation_text}: extracted_case_name='{result.get('extracted_case_name')}', extracted_date='{result.get('extracted_date')}'"
    )
    return result
    ...
    def _load_config(self):
        ...
        try:
            ...
            self._config = {
                ...
                "contact": {"email": CONTACT_EMAIL, "subject_prefix": CONTACT_SUBJECT_PREFIX},
            }
    ...
    def _register_routes(self, app: Any) -> None:
        ...
        def memory_status():
            ...
            if memory_monitor_available:
                ...
                return jsonify({"memory_monitor_available": True, "stats": stats})
            else:
                return jsonify({"memory_monitor_available": False, "message": "Memory monitoring not available"})
        ...
        def force_garbage_collection():
            ...
            if memory_monitor_available:
                freed_mb = force_gc()
                return jsonify({"success": True, "freed_mb": freed_mb, "message": f"Freed {freed_mb:.1f}MB of memory"})
            else:
                return jsonify({"success": False, "message": "Memory monitoring not available"})
...
def main():
    ...
    _ = parser.add_argument("--debug", action="store_true", default=os.getenv("FLASK_DEBUG", "").lower() == "true")
