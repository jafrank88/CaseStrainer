...
param(
    ...
    # NEW: Smart Docker cleanup parameters (from cslauncher)
    [Parameter()]
    ...
    # NEW: Worker management parameters (from cslauncher)
    [Parameter()]
    ...
    # Existing monitoring parameters
    [Parameter()]
    ...
    [Parameter()]
    [int]$MonitorInterval = 60,  # Default: 60 seconds
    
    [Parameter()]
    [int]$DockerDaemonTimeout = 15,  # Docker daemon freeze timeout in seconds (default: 15)
    
    [Parameter()]
    [int]$MaxDockerRestartsPerHour = 5,  # Maximum Docker daemon restarts per hour (default: 5)
    
    [Parameter()]
    [int]$ExtendedDowntimeMinutes = 15  # After this many minutes of downtime, bypass rate limit
)
...
# NEW: Enhanced logging paths (from cslauncher)
$reloadLogPath = Join-Path $PSScriptRoot "logs\reload.log"
$dockerDiagnosticsLogPath = Join-Path $PSScriptRoot "logs\docker_diagnostics.log"
$dockerHealthcheckLogPath = Join-Path $PSScriptRoot "logs\docker_healthchecks.log"
...
# NEW: Worker topology (single source of truth for worker operations - from cslauncher)
...
# NEW: Enhanced logging functions (from cslauncher)
function Write-ReloadLog {
    param(
        [string]$Message,
        [string]$Level = "INFO"
    )
    if ($LogErrors -or $Level -in @("ERROR", "WARN", "SUCCESS")) {
        $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        $logEntry = "[$timestamp] [$Level] $Message"
...
# NEW: Smart Docker cleanup functions (from cslauncher)
function Get-DockerMemoryUsage {
    # Get Docker memory usage in GB
    try {
        $dockerStats = docker system df --format "{{.Type}}:{{.Size}}" 2>$null
        $totalSize = 0
        
        foreach ($line in $dockerStats) {
            if ($line -match "Images|Containers|Local Volumes") {
                if ($line -match ":(\d+\.?\d*)([KMG]B)?") {
        ...
        Write-ReloadLog "Failed to get Docker memory usage: $($_.Exception.Message)" "WARN"
        return $null
    }
}
...
function Get-DockerDiskUsage {
    ...
    try {
        $dockerInfo = docker system df --format "table {{.Type}}\t{{.TotalCount}}\t{{.ActiveCount}}\t{{.Size}}\t{{.Reclaimable}}" 2>$null
        $totalSize = 0
        ...
        Write-ReloadLog "Failed to get Docker disk usage: $($_.Exception.Message)" "WARN"
        return $null
    }
}
...
function Invoke-SmartDockerCleanup {
    ...
    if (-not $needsCleanup) {
        ...
        Write-ReloadLog "   Current usage: Memory=${memUsage}GB, Disk=${diskUsage}GB" "INFO"
        Write-ReloadLog "   Thresholds: Memory=${MemoryThresholdGB}GB, Disk=${DiskThresholdGB}GB" "INFO"
        
        if ($memUsage -and $memUsage -gt $MemoryThresholdGB) {
            $needsCleanup = $true
            $reason = "memory usage ${memUsage}GB > ${MemoryThresholdGB}GB"
        }
        
        if (-not $needsCleanup -and $diskUsage -and $diskUsage -gt $DiskThresholdGB) {
            $needsCleanup = $true
            $reason = "disk usage ${diskUsage}GB > ${DiskThresholdGB}GB"
        }
    }
    
    if ($needsCleanup) {
        Write-ReloadLog "   Cleanup needed: $reason" "WARN"
        Write-Host "   Docker cleanup needed: $reason" -ForegroundColor Yellow
        
        try {
            Write-Host "   Pruning build cache..." -ForegroundColor Gray
            docker builder prune -f 2>&1 | Out-Null
            Write-ReloadLog "   Build cache pruned" "INFO"
            
            Write-Host "   Pruning unused images..." -ForegroundColor Gray
            docker image prune -f 2>&1 | Out-Null
            Write-ReloadLog "   Unused images pruned" "INFO"
            
            Write-Host "   Pruning unused containers..." -ForegroundColor Gray
            docker container prune -f 2>&1 | Out-Null
            Write-ReloadLog "   Unused containers pruned" "INFO"
            
            Write-Host "   Pruning unused volumes..." -ForegroundColor Gray
            docker volume prune -f 2>&1 | Out-Null
            Write-ReloadLog "   Unused volumes pruned" "INFO"
            
            Write-ReloadLog "   Cleanup complete" "SUCCESS"
            Write-Host "   Docker cleanup complete" -ForegroundColor Green
        } catch {
            Write-ReloadLog "   Cleanup failed: $($_.Exception.Message)" "ERROR"
            Write-Host "   ERROR: Docker cleanup failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    } else {
        Write-ReloadLog "   No cleanup needed - resources within limits" "INFO"
        Write-Host "   No Docker cleanup needed - resources within limits" -ForegroundColor Green
    }
}
...
function Send-StartupNotification {
    ...
    Write-ReloadLog "Startup notification would be sent for type: $StartupType" "INFO"
    return
}
...
# NEW: Smart Docker cleanup (from cslauncher)
# Step 1: Smart Docker cleanup (if needed or forced)
if ($CleanDocker) {
    Write-Host "[CLEANUP] Force cleanup requested..." -ForegroundColor Yellow
...
if ($containers.Count -gt 0 -and -not $Build -and -not $Force) {
    ...
    if ($needsFrontendRebuild) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  [OK] All $script:WorkerCount workers restarted" -ForegroundColor Green
        ...
        # NEW: Build all 6 workers (from cslauncher)
        docker-compose -f docker-compose.prod.yml build --no-cache backend $script:WorkerServices
        
        # NEW: Worker-first restart order (critical for code reload - from cslauncher)
        Write-Host "`n[RESTART] Using worker-first restart order (critical for code reload)..." -ForegroundColor Cyan
        Write-Host "   Restarting all $script:WorkerCount RQ workers (--force-recreate so each runs the new image)..." -ForegroundColor Gray
