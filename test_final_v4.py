...
r = requests.get('http://localhost:5000/casestrainer/api/task_status/d9ea809c-eb7d-4991-b6cb-dd8140dde1cc', timeout=30)
d = r.json()
...
for c in clusters:
    ...
    if 'meri' in name.lower():
        print(f"\nCluster: {name}")
        print(f"  cluster_id: {c.get('cluster_id')}")
        print(f"  size: {len(c.get('citations', []))}")
        for cit in c.get('citations', []):
            print(f"  - {cit.get('citation')}: verified={cit.get('verified')}, source={cit.get('source')}")
